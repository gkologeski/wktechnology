CREATE OR REPLACE FUNCTION public.get_entity_field_catalog(
  p_table text,
  p_owner_id uuid
)
RETURNS TABLE(
...
DECLARE
  ...
  cnt int;
BEGIN
  IF p_table NOT IN ('leads','contacts','companies','deals') THEN
...
GRANT EXECUTE ON FUNCTION public.get_entity_field_catalog(text, uuid) TO authenticated;