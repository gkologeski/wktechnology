...
ALTER TABLE public.calendar_events ADD COLUMN IF NOT EXISTS recurring_event_id text;
CREATE INDEX IF NOT EXISTS calendar_events_recurring_event_idx
  ON public.calendar_events (calendar_account_id, recurring_event_id)
  WHERE recurring_event_id IS NOT NULL;

-- Helper: true if the calendar_event belongs to a series with 6+ instances
-- (or an open-ended series, which the Google API expands into many instances).
CREATE OR REPLACE FUNCTION public._calendar_event_is_long_series(p_event_id uuid)
RETURNS boolean
LANGUAGE sql
STABLE
...
AS $$
  WITH e AS (
    SELECT calendar_account_id, recurring_event_id, title
      FROM calendar_events WHERE id = p_event_id
  ),
  sib AS (
    SELECT 1 FROM calendar_events s, e
    WHERE s.calendar_account_id = e.calendar_account_id
      AND (
        (e.recurring_event_id IS NOT NULL AND s.recurring_event_id = e.recurring_event_id)
        OR (e.recurring_event_id IS NULL
            AND e.title IS NOT NULL AND e.title <> ''
            AND s.recurring_event_id IS NULL
            AND s.title = e.title)
      )
    LIMIT 6
  )
  SELECT (SELECT count(*) FROM sib) >= 6;
$$;
...
BEGIN
  ...
  WITH base AS (
    ...
    UNION ALL
    -- calendar_events for contact timeline: match by direct contact link AND
    -- require WK to actually participate (at least one attendee on a WK
    -- internal domain). Also exclude events that belong to long/open recurring
    -- series (>= 6 occurrences) — internal client routines pollute the timeline.
    SELECT
    ...
    WHERE p_entity_kind = 'contact'
      ...
      AND EXISTS (
        SELECT 1
        FROM jsonb_array_elements(COALESCE(ce.attendees, '[]'::jsonb)) att
        WHERE lower(att->>'email') LIKE '%@wktechnology.com.br'
           OR lower(att->>'email') LIKE '%@wkconsultoria.com.br'
      )
    ...
    -- client contact — require WK participation AND exclude long/open
    -- recurring series (>= 6 occurrences).
    SELECT
      ('cal_' || ce.id::text), 'calendar_event', 'meeting',
      ce.title, NULL,
      ce.start_at,
