...
DECLARE
  ...
  v_jobs text[][] := ARRAY[
    ...
    ['email-broadcast-tick',    '*/2 * * * *',  '/api/public/hooks/email-broadcast-tick'],
    ['gmail-inbound-sync-tick', '*/5 * * * *',  '/api/public/hooks/email-sync-tick'],
    ['hubspot-import-tick',     '*/10 * * * *', '/api/public/hooks/hubspot-tick'],
    ['scheduled-exports-tick',  '0 * * * *',    '/api/public/hooks/scheduled-exports-tick'],
    ['scoring-tick-minutely',   '*/5 * * * *',  '/api/public/hooks/scoring-tick'],
    ['sentiment-tick',          '*/5 * * * *',  '/api/public/hooks/sentiment-tick'],
    ['sequences-tick',          '*/2 * * * *',  '/api/public/hooks/sequences-tick'],
    ['webhook-dispatch-tick',   '*/2 * * * *',  '/api/public/hooks/webhook-tick'],
    ['whatsapp-campaign-tick',  '*/2 * * * *',  '/api/public/hooks/whatsapp-campaign-tick'],
    ['workflows-tick',          '*/2 * * * *',  '/api/public/hooks/workflows-tick'],
...
REVOKE ALL ON FUNCTION public.reschedule_lovable_cron(text) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.reschedule_lovable_cron(text) TO service_role;