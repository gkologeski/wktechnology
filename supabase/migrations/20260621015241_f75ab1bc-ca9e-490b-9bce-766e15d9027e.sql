...
DECLARE
  v_count int := 0;
  v_allowed text[] := ARRAY['related_contact_id','related_company_id','related_deal_id','related_lead_id','related_ticket_id'];
BEGIN
  IF NOT (p_filter_col = ANY(v_allowed)) OR NOT (p_set_col = ANY(v_allowed)) THEN
    RAISE EXCEPTION 'invalid column';
  END IF;
  IF p_filter_col = p_set_col THEN RETURN 0; END IF;

  EXECUTE format($q$
    WITH cte AS (
      SELECT id FROM public.activities
      WHERE %I = $1 AND %I IS NULL
