...
-- Restrict calendar_events mirrored into contact/deal/company timelines to
-- events where WK actually participates (at least one attendee on a WK
-- internal domain). No time-window filtering.
...
BEGIN
  ...
  WITH base AS (
    ...
    -- internal domain). Filters out client-only internal meetings that
    -- happened to invite our contact.
    SELECT
    ...
    -- client contact — additionally require that WK actually participates
    -- (at least one attendee on a WK internal domain). Prevents client-only
    -- internal recurring meetings from polluting deal/company timelines.
    SELECT
      ('cal_' || ce.id::text), 'calendar_event', 'meeting',
      ce.title, NULL,
      ce.start_at,
...
-- Backfill: break the related_contact_id link on calendar_events that never
-- had a WK attendee. Data-only, no row deletion, no time-window filter.
UPDATE public.calendar_events
SET related_contact_id = NULL
WHERE related_contact_id IS NOT NULL
  AND NOT EXISTS (
    SELECT 1
    FROM jsonb_array_elements(COALESCE(attendees, '[]'::jsonb)) att
    WHERE lower(att->>'email') LIKE '%@wktechnology.com.br'
       OR lower(att->>'email') LIKE '%@wkconsultoria.com.br'
  );
