...
import { useWindowChrome, WindowChromeContext, type WindowChrome } from "@/components/activity/activity-window-context";
...
const Dialog = ({ onOpenChange, docked = false, ...props }: React.ComponentProps<typeof DialogPrimitive.Root> & { docked?: boolean }) => {
  ...
  return (
    ...
    <DialogPrimitive.Root
      ...
      modal={!dock}
      onOpenChange={(open) => {
        if (dock && !open) { dock.onClose(); return; }
...
>(({ className, children, ...props }, ref) => (
  <DockedDialogContent className={className} forwardedRef={ref} {...props}>{children}</DockedDialogContent>
));

function DockedDialogContent({ className, children, forwardedRef, ...props }: React.ComponentPropsWithoutRef<typeof DialogPrimitive.Content> & { forwardedRef: React.ForwardedRef<React.ElementRef<typeof DialogPrimitive.Content>> }) {
  ...
  <DialogPortal>
    ...
    >
      ...
      {!dock && <DialogPrimitive.Close className="absolute right-5 top-5 z-10 inline-flex h-8 w-8 items-center justify-center rounded-full text-muted-foreground transition-colors hover:bg-muted hover:text-foreground focus:outline-none focus-visible:ring-2 focus-visible:ring-ring disabled:pointer-events-none cursor-pointer">
        <X className="h-4 w-4" />
