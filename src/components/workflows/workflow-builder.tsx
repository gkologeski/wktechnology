      ...
      <SheetContent className="w-full sm:max-w-2xl overflow-y-auto">
        ...
        <div className="space-y-6 py-4">
          ...
          <div className="grid grid-cols-[1fr_auto] gap-3 items-end">
            <div>
              <Label>Nome</Label>
              <Input value={state.name} onChange={(e) => setState((s) => ({ ...s, name: e.target.value }))} placeholder="Ex: Notificar comercial quando lead novo" />
            </div>
          ...
          <section className="space-y-3 rounded-md border p-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <Select value={state.entity} onValueChange={(v) => setState((s) => ({ ...s, entity: v as WorkflowEntity }))}>
              ...
              <div>
                <Label>Evento</Label>
                <Select value={state.trigger.event} onValueChange={(v) => setState((s) => ({ ...s, trigger: { ...s.trigger, event: v as WorkflowTrigger["event"] } }))}>
                  <SelectTrigger><SelectValue /></SelectTrigger>
                  <SelectContent>
          ...
          <section className="space-y-3 rounded-md border p-4">
            <div className="flex items-center justify-between">
              ...
              <Button variant="ghost" size="sm" onClick={() => setFilters((p) => [...(p ?? []), { field: fields[0], op: "eq", value: "" }])}>
            ...
            {(state.trigger.filters ?? []).map((f, i) => {
              ...
              return (
                <div key={i} className="grid grid-cols-[1fr_1fr_1fr_auto] gap-2">
                  <Select value={f.field} onValueChange={(v) => setFilters((p) => p!.map((x, idx) => idx === i ? { ...x, field: v } : x))}>
                    <SelectTrigger><SelectValue /></SelectTrigger>
                    <SelectContent>{fields.map((ff) => <SelectItem key={ff} value={ff}>{ff}</SelectItem>)}</SelectContent>
                  </Select>
                  <Select value={f.op} onValueChange={(v) => setFilters((p) => p!.map((x, idx) => idx === i ? { ...x, op: v as FilterOp } : x))}>
                    <SelectTrigger><SelectValue /></SelectTrigger>
                    <SelectContent>{FILTER_OPS.map((o) => <SelectItem key={o.value} value={o.value}>{o.label}</SelectItem>)}</SelectContent>
                  </Select>
                  {needsValue ? (
                    <Input value={String(f.value ?? "")} onChange={(e) => setFilters((p) => p!.map((x, idx) => idx === i ? { ...x, value: e.target.value } : x))} placeholder="valor" />
                  ) : (<div />)}
                  <Button variant="ghost" size="icon" onClick={() => setFilters((p) => p!.filter((_, idx) => idx !== i))}>
          ...
          <section className="space-y-3 rounded-md border p-4">
            <div className="flex items-center justify-between">
              ...
              <Select onValueChange={(v) => setActions((p) => [...p, defaultActionOfType(v as WorkflowActionType)])}>
                <SelectTrigger className="w-[200px]"><SelectValue placeholder="+ Adicionar ação" /></SelectTrigger>
                <SelectContent>
            ...
            <div className="space-y-3">
              {state.actions.map((a, i) => (
                <ActionCard
                  ...
                  onMoveUp={i > 0 ? () => setActions((p) => {
                    const c = [...p]; [c[i - 1], c[i]] = [c[i], c[i - 1]]; return c;
                  }) : undefined}
                />
              ))}
            </div>
        ...
        <SheetFooter>
          ...
          <Button onClick={handleSave} disabled={saving || !state.name || state.actions.length === 0}>
...
function ActionCard({
  action, entityFields, onChange, onRemove, onMoveUp,
}: {
  ...
  return (
    <div className="rounded-md border p-3 space-y-2 bg-muted/20">
      <div className="flex items-center gap-2">
        <button type="button" onClick={onMoveUp} disabled={!onMoveUp} className="text-muted-foreground disabled:opacity-30">
      ...
      {action.type === "set_field" && (
        <div className="grid grid-cols-2 gap-2">
          <Select value={action.field} onValueChange={(v) => onChange({ ...action, field: v })}>
            <SelectTrigger><SelectValue /></SelectTrigger>
            <SelectContent>{entityFields.map((f) => <SelectItem key={f} value={f}>{f}</SelectItem>)}</SelectContent>
          </Select>
          <Input value={String(action.value ?? "")} onChange={(e) => onChange({ ...action, value: e.target.value })} placeholder="novo valor" />
        </div>
      )}
      ...
      {action.type === "create_activity" && (
        <div className="space-y-2">
          <div className="grid grid-cols-[1fr_120px] gap-2">
            <Input value={action.subject} onChange={(e) => onChange({ ...action, subject: e.target.value })} placeholder="Assunto" />
            <Select value={action.activity_type ?? "task"} onValueChange={(v) => onChange({ ...action, activity_type: v })}>
              <SelectTrigger><SelectValue /></SelectTrigger>
              <SelectContent>
          ...
          <Textarea value={action.body ?? ""} onChange={(e) => onChange({ ...action, body: e.target.value })} placeholder="Descrição (opcional)" rows={2} />
          <div className="flex items-center gap-2">
            <Label className="text-xs">Vence em (dias)</Label>
            <Input type="number" min={0} max={365} className="w-24"
              value={action.due_in_days ?? ""} onChange={(e) => onChange({ ...action, due_in_days: e.target.value ? Number(e.target.value) : undefined })} />
          </div>
        </div>
      )}
      ...
      {action.type === "send_notification" && (
        <div className="space-y-2">
          <Input value={action.title} onChange={(e) => onChange({ ...action, title: e.target.value })} placeholder="Título" />
          <Textarea value={action.body ?? ""} onChange={(e) => onChange({ ...action, body: e.target.value })} placeholder="Corpo (opcional)" rows={2} />
        </div>
      )}

      {action.type === "webhook" && (
        <div className="space-y-2">
          <Input value={action.url} onChange={(e) => onChange({ ...action, url: e.target.value })} placeholder="https://..." />
          <Textarea value={JSON.stringify(action.payload ?? {}, null, 2)}
            onChange={(e) => {
              try { onChange({ ...action, payload: JSON.parse(e.target.value) }); }
              catch { /* ignore until válido */ }
            }}
            placeholder='{"foo": "bar"}' rows={3} className="font-mono text-xs" />
        </div>
      )}
    </div>
  );
