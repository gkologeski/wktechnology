...
export function DashboardFilters({
  ...
  return (
    <div className="flex flex-wrap items-end gap-3 rounded-md border border-product-divider bg-product-toolbar p-3">
      ...
      <div className="min-w-[12rem]">
        <Label htmlFor="dash-lead-pipeline" className="text-xs text-text-secondary">Funil de Leads</Label>
        <Select value={leadPipelineId ?? "__default__"} onValueChange={(v) => onLeadPipelineChange(v === "__default__" ? null : v)} disabled={disabled || leadPipelines.length === 0}>
          <SelectTrigger id="dash-lead-pipeline" className="mt-1 h-9 w-full"><SelectValue placeholder="Funil padrão" /></SelectTrigger>
          <SelectContent>
            <SelectItem value="__default__">Funil padrão</SelectItem>
            {leadPipelines.map((p) => <SelectItem key={p.id} value={p.id}>{p.name}</SelectItem>)}
          </SelectContent>
        </Select>
      </div>

      <div className="min-w-[11rem]">
        <Label htmlFor="dash-channel" className="text-xs text-text-secondary">Canal</Label>
        <Select value={channel ?? "__all__"} onValueChange={(v) => onChannelChange(v === "__all__" ? null : v as LeadChannel)} disabled={disabled}>
          <SelectTrigger id="dash-channel" className="mt-1 h-9 w-full"><SelectValue /></SelectTrigger>
          <SelectContent>
            <SelectItem value="__all__">Todos os canais</SelectItem>
            <SelectItem value="prospecting">Prospecção</SelectItem><SelectItem value="website">Site e formulários</SelectItem>
            <SelectItem value="paid">Mídia paga</SelectItem><SelectItem value="organic">Orgânico</SelectItem>
            <SelectItem value="referral">Indicação</SelectItem><SelectItem value="offline">Eventos e offline</SelectItem>
            <SelectItem value="import">Importação</SelectItem><SelectItem value="other">Outros</SelectItem><SelectItem value="unknown">Sem origem</SelectItem>
          </SelectContent>
        </Select>
      </div>
