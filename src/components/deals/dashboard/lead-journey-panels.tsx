...
export function LeadJourneyKpis({ journey }: { journey: LeadJourneyData }) {
  return (
    <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-4">
      <MetricCard label="Leads novos" value={journey.totalLeads.toLocaleString("pt-BR")} hint="Entradas no período" icon={ContactRound} />
      <MetricCard label="Qualificados" value={journey.qualified.toLocaleString("pt-BR")} hint={`${pct(journey.leadToQualifiedRate)} dos leads`} icon={CircleCheck} tone="positive" />
      <MetricCard label="Oportunidades" value={journey.opportunities.toLocaleString("pt-BR")} hint={`${pct(journey.qualifiedToOpportunityRate)} dos qualificados`} icon={Target} tone="warning" />
      <MetricCard label="Vendas atribuídas" value={journey.sales.toLocaleString("pt-BR")} hint={`${pct(journey.opportunityToSaleRate)} das oportunidades · ${formatCurrency(journey.revenue)}`} icon={BadgeDollarSign} tone="positive" />
    </div>
  );
}
...
export function LeadJourneyOverview({ journey }: { journey: LeadJourneyData }) {
  const steps = [
    ...
    { label: "Oportunidades", value: journey.opportunities, rate: journey.qualifiedToOpportunityRate },
  ...
  return (
    <section className="rounded-lg border border-border-subtle bg-surface-1 p-4 md:p-5">
      <SectionHeader title="Caminho do lead até a venda" description="Origem principal e conversão da coorte criada no período." />
      <ol className="mt-5 grid gap-3 md:grid-cols-4" aria-label="Jornada de conversão">
        {steps.map((step, index) => (
          <li key={step.label} className="relative min-w-0 rounded-md border border-border-subtle bg-surface-2 p-4">
            <div className="text-xs font-medium text-text-secondary">{step.label}</div>
            <div className="mt-1 text-2xl font-semibold tabular-nums text-text-primary">{step.value.toLocaleString("pt-BR")}</div>
            <div className="mt-2 min-h-5 text-xs text-text-tertiary">{step.rate == null ? "Entrada da jornada" : `${pct(step.rate)} da etapa anterior`}</div>
            {index < steps.length - 1 ? <ArrowRight className="absolute -right-5 top-1/2 z-10 hidden h-5 w-5 -translate-y-1/2 text-text-tertiary md:block" aria-hidden /> : null}
          </li>
        ))}
      </ol>
      <div className="mt-4 flex flex-wrap items-center justify-between gap-2 border-t border-border-subtle pt-3 text-xs text-text-secondary">
        <span>{journey.attributionCoverage.toFixed(1)}% das conversões possuem negócio rastreável.</span>
...
export function LeadChannelsPanel({ journey, onSelect }: { journey: LeadJourneyData; onSelect: (channel: LeadChannel | null) => void }) {
