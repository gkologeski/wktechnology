      ...
      <div className="max-w-3xl mx-auto space-y-4">
        ...
        <Card>
          ...
          <CardContent className="space-y-2">
            {signers.map((s) => (
              <div key={s.id} className={`flex items-center justify-between rounded border p-2 ${s.id === signer.id ? "border-primary bg-primary/5" : ""}`}>
                <div>
                  <div className="text-sm font-medium">{s.name} {s.id === signer.id && <span className="text-xs text-primary">(você)</span>}</div>
                  <div className="text-xs text-muted-foreground">{s.email} · ordem {s.sign_order}</div>
                </div>
        ...
        {alreadySigned ? (
          ...
          <Card className="border-destructive">
            <CardContent className="py-6 flex items-center gap-3">
              ...
              <div>
                ...
                {signer.decline_reason && <div className="text-xs text-muted-foreground">{signer.decline_reason}</div>}
              </div>
            </CardContent>
          </Card>
        ) : !canSign ? (
          <Card>
            <CardContent className="py-6 text-muted-foreground text-sm">
              Aguardando assinatura dos signatários anteriores. Você será notificado quando for sua vez.
            </CardContent>
          </Card>
        ) : showDecline ? (
          <Card>
            <CardHeader><CardTitle className="text-base">Recusar assinatura</CardTitle></CardHeader>
            <CardContent className="space-y-3">
              <Label>Motivo (opcional)</Label>
              <Textarea rows={3} value={reason} onChange={(e) => setReason(e.target.value)} />
              <div className="flex gap-2">
                <Button variant="outline" onClick={() => setShowDecline(false)}>Voltar</Button>
                <Button variant="destructive" onClick={() => declineMut.mutate()} disabled={declineMut.isPending}>
          ...
          <Card>
            <CardHeader><CardTitle className="text-base">Assinar eletronicamente</CardTitle></CardHeader>
            <CardContent className="space-y-3">
              ...
              <label className="flex items-start gap-2 text-sm">
                ...
                <span>Li o documento acima e concordo em assiná-lo eletronicamente. Entendo que esta assinatura tem validade legal e ficará registrada com meu IP e horário.</span>
              </label>
              <div className="flex gap-2 justify-end">
                <Button variant="outline" onClick={() => setShowDecline(true)}>Recusar</Button>
                <Button onClick={() => signMut.mutate()} disabled={!accept || name.length < 2 || signMut.isPending}>
