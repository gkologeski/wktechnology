...
import {
  listEsignDocuments, createEsignDocument, sendEsignDocument,
  cancelEsignDocument, deleteEsignDocument, getEsignDocument,
  addEsignSigner, removeEsignSigner,
...
const STATUS_LABEL: Record<string, string> = {
  draft: "Rascunho", sent: "Enviado", partially_signed: "Parcial",
  completed: "Concluído", declined: "Recusado", expired: "Expirado", canceled: "Cancelado",
};
const STATUS_VARIANT: Record<string, "default" | "secondary" | "destructive" | "outline"> = {
  draft: "outline", sent: "secondary", partially_signed: "secondary",
  completed: "default", declined: "destructive", expired: "outline", canceled: "outline",
};
const SIGNER_LABEL: Record<string, string> = {
  pending: "Pendente", viewed: "Visualizado", signed: "Assinado", declined: "Recusado",
};
...
function EsignPage() {
  ...
  const [draft, setDraft] = useState({
    title: "", description: "", body: "", ordered: false,
  ...
  const createMut = useMutation({
    ...
    onSuccess: () => {
      ...
      setOpenCreate(false);
      setDraft({ title: "", description: "", body: "", ordered: false, signers: [{ name: "", email: "", sign_order: 1 }] });
  ...
  const pending = docs.filter((d) => ["sent", "partially_signed", "draft"].includes(d.status)).length;

  return (
    <div className="space-y-4">
      <div className="grid gap-3 md:grid-cols-3">
        <Card><CardHeader className="pb-2"><CardTitle className="text-sm">Total</CardTitle></CardHeader>
          <CardContent className="text-2xl font-semibold">{total}</CardContent></Card>
        <Card><CardHeader className="pb-2"><CardTitle className="text-sm">Concluídos</CardTitle></CardHeader>
          <CardContent className="text-2xl font-semibold">{completed}</CardContent></Card>
        <Card><CardHeader className="pb-2"><CardTitle className="text-sm">Em aberto</CardTitle></CardHeader>
          <CardContent className="text-2xl font-semibold">{pending}</CardContent></Card>
      </div>
          ...
          <DialogContent className="max-w-2xl">
            ...
            <div className="space-y-3 max-h-[70vh] overflow-y-auto pr-1">
              <div className="space-y-1.5">
                <Label>Título *</Label>
                <Input value={draft.title} onChange={(e) => setDraft({ ...draft, title: e.target.value })} placeholder="Contrato de prestação de serviços" />
              </div>
              <div className="space-y-1.5">
              ...
              <div className="flex items-center justify-between rounded-md border p-3">
                <div>
                  ...
                  <div className="text-xs text-muted-foreground">Cada signatário só assina depois do anterior.</div>
                </div>
                <Switch checked={draft.ordered} onCheckedChange={(v) => setDraft({ ...draft, ordered: v })} />
              </div>
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  <Label>Signatários *</Label>
                  <Button size="sm" variant="outline" onClick={() =>
                    setDraft({ ...draft, signers: [...draft.signers, { name: "", email: "", sign_order: draft.signers.length + 1 }] })
            ...
            <DialogFooter>
              ...
              <Button onClick={() => createMut.mutate()} disabled={createMut.isPending || !draft.title || !draft.body}>
      ...
      {isLoading ? (
        ...
        <div className="space-y-2">
          {docs.map((d) => {
            const signers = (d.esign_signers ?? []) as Array<{ id: string; name: string; status: string }>;
            const signed = signers.filter((s) => s.status === "signed").length;
            return (
              <div key={d.id} className="rounded-md border p-3 flex items-center justify-between gap-3 hover:bg-accent/30 cursor-pointer"
  ...
  return (
    <Sheet open={!!id} onOpenChange={(o) => !o && onClose()}>
      <SheetContent className="w-[90vw] sm:max-w-2xl overflow-y-auto">
        <SheetHeader><SheetTitle>{data?.doc?.title ?? "Documento"}</SheetTitle></SheetHeader>
        {isLoading || !data ? (
          ...
          <div className="space-y-5 mt-4">
            <div>
              ...
              <div className="rounded-md border bg-muted/30 p-3 whitespace-pre-wrap text-sm max-h-60 overflow-y-auto">{data.doc.body}</div>
            </div>
              ...
              <div className="space-y-2">
                {data.signers.map((s) => (
                  <div key={s.id} className="rounded-md border p-2 space-y-1">
                    <div className="flex items-center justify-between">
                      <div className="min-w-0">
                        <div className="font-medium text-sm">{s.name} <span className="text-xs text-muted-foreground">#{s.sign_order}</span></div>
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        <Button size="icon" variant="ghost" onClick={() => window.open(publicUrl(s.public_token), "_blank")}>
            ...
            <div>
              <div className="text-xs uppercase text-muted-foreground mb-2 flex items-center gap-1"><Eye className="h-3.5 w-3.5" /> Trilha de auditoria</div>
