...
const PLAN_LIMITS: Record<string, Record<string, number>> = {
  ...
  enterprise: { emails_sent: 500000, whatsapp_sent: 100000, api_requests: 1000000, ai_credits: 100000 },
};

const COUNTER_KEYS = ["emails_sent", "whatsapp_sent", "api_requests", "ai_credits"] as const;
const COUNTER_LABELS: Record<string, string> = {
  emails_sent: "Emails", whatsapp_sent: "WhatsApp", api_requests: "API", ai_credits: "IA",
};
  ...
  return (
    <div className="space-y-1 min-w-[140px]">
      <div className="flex justify-between text-xs">
        ...
        <Badge variant={color as any} className="h-4 text-[10px]">{pct}%</Badge>
      </div>
  ...
  return (
    <div className="p-6 space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">Uso mensal versus limite do plano. Alerta automático em 80%.</p>
      </div>
      <Card>
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  {COUNTER_KEYS.map((k) => <TableHead key={k}>{COUNTER_LABELS[k]}</TableHead>)}
                </TableRow>
              </TableHeader>
              <TableBody>
                {(data?.items ?? []).map((w: any) => {
                  ...
                  return (
                    <TableRow key={w.id}>
                      <TableCell><div className="font-medium">{w.name}</div><div className="text-xs text-muted-foreground">{w.slug}</div></TableCell>
                      <TableCell><Badge variant="outline">{w.plan}</Badge></TableCell>
                      {COUNTER_KEYS.map((k) => (
                        <TableCell key={k}>
