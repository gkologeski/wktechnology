...
import { listBookingPages, upsertBookingPage, deleteBookingPage, listBookings, cancelBooking } from "@/lib/booking.functions";
...
function BookingSettings() {
  ...
  const delMut = useMutation({
    ...
    onSuccess: () => { toast.success("Página excluída"); qc.invalidateQueries({ queryKey: ["booking-pages"] }); },
  });
    ...
    <div className="container max-w-6xl py-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Crie links públicos para clientes reservarem horários na sua agenda.</p>
        </div>
        <Button onClick={newPage}><Plus className="w-4 h-4 mr-1" /> Nova página</Button>
      </div>

      <div className="grid gap-3">
        {pages.length === 0 && <p className="text-sm text-muted-foreground">Nenhuma página criada ainda.</p>}
        {pages.map((p: any) => {
          ...
          return (
            <Card key={p.id} className="p-4 flex items-center gap-4">
              ...
              <div className="flex items-center gap-1">
                ...
                <Button size="sm" variant="ghost" onClick={() => { if (confirm("Excluir página?")) delMut.mutate(p.id); }}>
...
function EditorDialog({ initial, accounts, onClose, onSave }: {
  initial: any;
  accounts: any[];
  onClose: () => void;
  ...
  const updAvail = (key: string, wins: Win[]) => setForm((f: any) => ({ ...f, availability: { ...f.availability, [key]: wins } }));

  return (
    <Dialog open onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader><DialogTitle>{initial.id ? "Editar página" : "Nova página"}</DialogTitle></DialogHeader>
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="space-y-1">
              <Label>Slug *</Label>
              <Input value={form.slug} onChange={(e) => upd({ slug: e.target.value.toLowerCase().replace(/[^a-z0-9-]/g, "-") })} placeholder="reuniao-30min" />
            </div>
          </div>
          <div className="space-y-1">
          ...
          <div className="grid grid-cols-3 gap-3">
            <div className="space-y-1">
              ...
              <Input type="number" min={5} max={480} value={form.duration_minutes} onChange={(e) => upd({ duration_minutes: parseInt(e.target.value || "30", 10) })} />
            </div>
            <div className="space-y-1">
              <Label>Buffer antes</Label>
              <Input type="number" min={0} max={240} value={form.buffer_before_minutes} onChange={(e) => upd({ buffer_before_minutes: parseInt(e.target.value || "0", 10) })} />
            </div>
            <div className="space-y-1">
              <Label>Buffer depois</Label>
              <Input type="number" min={0} max={240} value={form.buffer_after_minutes} onChange={(e) => upd({ buffer_after_minutes: parseInt(e.target.value || "0", 10) })} />
            </div>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1">
              <Label>Antecedência mínima (h)</Label>
              <Input type="number" min={0} max={720} value={form.min_notice_hours} onChange={(e) => upd({ min_notice_hours: parseInt(e.target.value || "0", 10) })} />
            </div>
            <div className="space-y-1">
              <Label>Antecedência máxima (dias)</Label>
              <Input type="number" min={1} max={365} value={form.max_advance_days} onChange={(e) => upd({ max_advance_days: parseInt(e.target.value || "30", 10) })} />
            </div>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1">
            ...
            <div className="space-y-1">
              ...
              <Select value={form.calendar_account_id ?? "none"} onValueChange={(v) => upd({ calendar_account_id: v === "none" ? null : v })}>
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            <div className="space-y-1">
              ...
              <Input value={form.location ?? ""} onChange={(e) => upd({ location: e.target.value })} placeholder="Google Meet, sala, etc." />
            </div>
          </div>
                ...
                return (
                  <div key={d.key} className="flex items-start gap-3">
                    ...
                    <div className="flex-1 space-y-1">
                      {wins.length === 0 && <div className="text-xs text-muted-foreground py-1">Indisponível</div>}
                      {wins.map((w, i) => (
                        <div key={i} className="flex items-center gap-2">
                          <Input type="time" className="w-28" value={w.start} onChange={(e) => {
                            const next = [...wins]; next[i] = { ...w, start: e.target.value }; updAvail(d.key, next);
                          }} />
                          <span className="text-xs">até</span>
                          <Input type="time" className="w-28" value={w.end} onChange={(e) => {
                            const next = [...wins]; next[i] = { ...w, end: e.target.value }; updAvail(d.key, next);
                          }} />
                          <Button size="icon" variant="ghost" onClick={() => updAvail(d.key, wins.filter((_, j) => j !== i))}>
                            <X className="w-4 h-4" />
                          </Button>
                        </div>
                      ))}
                      <Button size="sm" variant="outline" onClick={() => updAvail(d.key, [...wins, { start: "09:00", end: "17:00" }])}>
                        + Janela
                      </Button>
                    </div>
                  </div>
...
function BookingsSheet({ pageId, onClose }: { pageId: string; onClose: () => void }) {
  ...
  const qc = useQueryClient();
  const { data = [] } = useQuery({ queryKey: ["bookings", pageId], queryFn: () => list({ data: { page_id: pageId } }) });

  return (
    <Sheet open onOpenChange={(o) => !o && onClose()}>
      <SheetContent className="w-full sm:max-w-xl overflow-y-auto">
        <SheetHeader><SheetTitle>Reservas</SheetTitle></SheetHeader>
        <div className="space-y-2 mt-4">
          {data.length === 0 && <p className="text-sm text-muted-foreground">Nenhuma reserva ainda.</p>}
          {data.map((b: any) => (
            <Card key={b.id} className="p-3">
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <div className="text-sm font-medium">
                    {new Date(b.start_at).toLocaleString(undefined, { dateStyle: "short", timeStyle: "short" })}
                  </div>
                  <div className="text-sm">{b.invitee_name} — {b.invitee_email}</div>
                  {b.invitee_phone && <div className="text-xs text-muted-foreground">{b.invitee_phone}</div>}
