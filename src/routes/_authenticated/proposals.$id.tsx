...
import {
  getProposal, updateProposal, sendProposal,
  requestProposalApproval, decideProposalApproval,
  listClauses,
...
const STATUS_LABEL: Record<string, string> = {
  draft: "Rascunho", in_review: "Em revisão", approved: "Aprovada", sent: "Enviada",
  accepted: "Aceita", rejected: "Recusada", expired: "Expirada", canceled: "Cancelada",
};
...
function ProposalEditor() {
  ...
  const saveM = useMutation({
    mutationFn: () => update({ data: { id, patch: { title, body, total_amount: amount ? Number(amount) : null } } }),
    onSuccess: () => { toast.success("Salvo"); qc.invalidateQueries({ queryKey: ["proposal", id] }); },
  ...
  const reqM = useMutation({
    ...
    onSuccess: () => { toast.success("Aprovação solicitada"); setComment(""); qc.invalidateQueries({ queryKey: ["proposal", id] }); },
    onError: (e: Error) => toast.error(e.message),
  });
  const decideM = useMutation({
    ...
    onSuccess: () => { toast.success("Decisão registrada"); setComment(""); qc.invalidateQueries({ queryKey: ["proposal", id] }); },
    onError: (e: Error) => toast.error(e.message),
  });
  ...
  return (
    <div className="grid grid-cols-1 gap-4 p-6 lg:grid-cols-[1fr_320px]">
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <Button variant="ghost" size="icon" asChild><Link to="/proposals"><ArrowLeft className="h-4 w-4" /></Link></Button>
            <div>
              ...
              <div className="flex items-center gap-2 text-xs text-muted-foreground">
                ...
                {locked && <span className="flex items-center gap-1"><Lock className="h-3 w-3" />Imutável</span>}
              </div>
            </div>
          </div>
          <div className="flex gap-2">
            <Button variant="outline" onClick={() => saveM.mutate()} disabled={locked || saveM.isPending}>
              <Save className="mr-2 h-4 w-4" />Salvar
            </Button>
            {prop.status === "draft" && (
              <Sheet>
                <SheetTrigger asChild><Button variant="secondary"><ShieldCheck className="mr-2 h-4 w-4" />Solicitar aprovação</Button></SheetTrigger>
                <SheetContent>
                  <SheetHeader><SheetTitle>Solicitar aprovação interna</SheetTitle></SheetHeader>
                  <div className="space-y-3 pt-4">
                    <Label>Comentário</Label>
                    <Textarea value={comment} onChange={(e) => setComment(e.target.value)} rows={4} />
                    <Button onClick={() => reqM.mutate()} disabled={reqM.isPending}>Enviar para revisão</Button>
                  </div>
                </SheetContent>
              </Sheet>
            )}
            {(prop.status === "approved" || prop.status === "draft") && (
              <Button onClick={() => sendM.mutate()} disabled={sendM.isPending}>
                <Send className="mr-2 h-4 w-4" />Enviar e selar
              </Button>
            )}
          </div>
        </div>
        ...
        <Card>
          <CardHeader><CardTitle>Conteúdo da proposta</CardTitle></CardHeader>
          <CardContent className="space-y-3">
            ...
            {locked ? (
              ...
              <RichHtmlEditor value={body} onChange={setBody} minHeight={360} placeholder="Escreva sua proposta… use {{deal.amount}}, {{contact.name}}, etc." />
            )}
            <div className="text-xs text-muted-foreground">Pré-visualização (tokens simples): <span className="ml-2 inline-block" dangerouslySetInnerHTML={{ __html: previewHtml.slice(0, 200) }} /></div>
          </CardContent>
        </Card>
                ...
                <div key={a.id} className="flex items-center justify-between rounded-md border p-2 text-sm">
                  ...
                  {a.status === "pending" && (
                    <div className="flex gap-1">
                      <Button size="sm" variant="outline" onClick={() => decideM.mutate({ approvalId: a.id, decision: "approved" })}>Aprovar</Button>
                      <Button size="sm" variant="destructive" onClick={() => decideM.mutate({ approvalId: a.id, decision: "rejected" })}>Reprovar</Button>
                    </div>
                  )}
                </div>
              ))}
      ...
      <aside className="space-y-4">
        <Card>
          <CardHeader><CardTitle className="text-sm">Variáveis</CardTitle></CardHeader>
          <CardContent className="space-y-1 text-xs">
            {["{{contact.name}}", "{{contact.email}}", "{{company.name}}", "{{company.cnpj}}", "{{deal.amount}}", "{{deal.title}}"].map((t) => (
              <button key={t} type="button" disabled={!!locked}
                onClick={() => setBody((b) => b + " " + t)}
                className="block w-full rounded px-2 py-1 text-left font-mono hover:bg-muted disabled:opacity-50">{t}</button>
            ))}
          </CardContent>
        </Card>
        <Card>
          <CardHeader>
            <CardTitle className="text-sm flex items-center justify-between">
              Cláusulas
              <Link to="/settings/clauses" className="text-xs text-primary hover:underline">Gerenciar</Link>
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-1 text-xs">
            {(clauses ?? []).length === 0 && <p className="text-muted-foreground">Biblioteca vazia.</p>}
            {(clauses ?? []).map((c) => (
