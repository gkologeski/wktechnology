...
import { listAlertRules, upsertAlertRule, deleteAlertRule, listAlertEvents } from "@/lib/platform-observability.functions";
...
function AdminAlertsPage() {
  ...
  const rules = useQuery({ queryKey: ["alert-rules"], queryFn: () => listFn(), enabled: isPlatformAdmin });
  const events = useQuery({ queryKey: ["alert-events"], queryFn: () => eventsFn(), enabled: isPlatformAdmin });

  const [open, setOpen] = useState(false);
  const [form, setForm] = useState({
    name: "", description: "", rule_type: "cron_late" as const, threshold_mins: 5,
    threshold_pct: 10, target_key: "", email: "", enabled: true,
  });

  const save = useMutation({
    mutationFn: () => upsertFn({ data: {
      name: form.name, description: form.description, rule_type: form.rule_type,
      threshold_mins: form.threshold_mins, threshold_pct: form.threshold_pct,
  ...
  const remove = useMutation({
    ...
    onSuccess: () => { toast.success("Regra removida"); qc.invalidateQueries({ queryKey: ["alert-rules"] }); },
  });
  ...
  return (
    <div className="p-6 space-y-6">
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          ...
          <DialogContent>
            ...
            <div className="space-y-3">
              ...
              <div>
                <Label>Tipo</Label>
                <Select value={form.rule_type} onValueChange={(v) => setForm({ ...form, rule_type: v as typeof form.rule_type })}>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div><Label>Limite (min)</Label><Input type="number" value={form.threshold_mins} onChange={(e) => setForm({ ...form, threshold_mins: Number(e.target.value) })} /></div>
                <div><Label>Limite (%)</Label><Input type="number" value={form.threshold_pct} onChange={(e) => setForm({ ...form, threshold_pct: Number(e.target.value) })} /></div>
              </div>
              <div><Label>Alvo (ex: nome do cron)</Label><Input value={form.target_key} onChange={(e) => setForm({ ...form, target_key: e.target.value })} /></div>
              <div><Label>Email de notificação</Label><Input type="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} /></div>
              <div className="flex items-center gap-2"><Switch checked={form.enabled} onCheckedChange={(c) => setForm({ ...form, enabled: c })} /><Label>Ativa</Label></div>
            </div>
            <DialogFooter>
      ...
      <Card>
        ...
        <CardContent>
          {(rules.data?.items ?? []).length === 0 ? <p className="text-sm text-muted-foreground">Nenhuma regra configurada.</p> : (
            <ul className="space-y-2">
              {(rules.data?.items ?? []).map((r: any) => (
                <li key={r.id} className="flex items-center justify-between border-b pb-2">
                  <div>
                    <div className="font-medium">{r.name} <Badge variant={r.enabled ? "default" : "secondary"}>{r.enabled ? "ativa" : "inativa"}</Badge></div>
                    <div className="text-xs text-muted-foreground">{r.rule_type} · alvo: {r.target_key || "—"} · {r.threshold_mins ?? "—"}min / {r.threshold_pct ?? "—"}%</div>
                  </div>
      ...
      <Card>
        ...
        <CardContent>
          {(events.data?.items ?? []).length === 0 ? <p className="text-sm text-muted-foreground">Sem eventos.</p> : (
            <ul className="space-y-2">
