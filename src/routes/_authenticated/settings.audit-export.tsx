...
import {
  listAuditExports, upsertAuditExport, deleteAuditExport, runAuditExportNow,
    ...
    <div className="p-6 space-y-6 max-w-4xl">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Envie logs para S3, webhook ou email em intervalos regulares.</p>
        </div>
        <Button onClick={() => setEditing({ destination: "webhook", format: "json", enabled: true, schedule_cron: "0 2 * * *", config: {} })}>
      ...
      <div className="space-y-3">
          ...
          <Card key={r.id}>
            <CardContent className="p-4 flex items-center justify-between gap-3">
              ...
              <div className="flex gap-1">
                <Button variant="ghost" size="sm" onClick={async () => { const x = await runNow({ data: { id: r.id } }); x.ok ? toast.success(`OK (${(x as any).count ?? 0} regs)`) : toast.error((x as any).error); load(); }}>
                  <Play className="h-4 w-4" />
                </Button>
                <Button variant="ghost" size="sm" onClick={() => setEditing(r)}><Pencil className="h-4 w-4" /></Button>
                <Button variant="ghost" size="sm" onClick={async () => { await del({ data: { id: r.id } }); load(); }}>
        ...
        {rows.length === 0 && <p className="text-sm text-muted-foreground">Nenhuma exportação configurada.</p>}
      </div>

      {editing && (
        <Card>
          <CardHeader><CardTitle>{editing.id ? "Editar" : "Nova"} exportação</CardTitle></CardHeader>
          <CardContent className="space-y-3">
            <Field label="Nome"><Input value={editing.name ?? ""} onChange={(e) => setEditing({ ...editing, name: e.target.value })} /></Field>
            <div className="grid grid-cols-2 gap-3">
              <Field label="Destino">
                <Select value={editing.destination} onValueChange={(v) => setEditing({ ...editing, destination: v as any })}>
            ...
            {editing.destination === "webhook" && (
              <Field label="URL do webhook">
                <Input value={editing.config?.url ?? ""} onChange={(e) => setEditing({ ...editing, config: { ...editing.config, url: e.target.value } })} />
              </Field>
            )}
            {editing.destination === "s3" && (
              <Field label="Presigned URL (PUT)">
                <Input value={editing.config?.presigned_url ?? ""} onChange={(e) => setEditing({ ...editing, config: { ...editing.config, presigned_url: e.target.value } })} />
              </Field>
            )}
            {editing.destination === "email" && (
              <Field label="Email destinatário">
                <Input value={editing.config?.email ?? ""} onChange={(e) => setEditing({ ...editing, config: { ...editing.config, email: e.target.value } })} />
              </Field>
            )}
            <Field label="Cron schedule"><Input value={editing.schedule_cron ?? ""} onChange={(e) => setEditing({ ...editing, schedule_cron: e.target.value })} /></Field>
            <Field label="HMAC secret (opcional, assina o body)">
              <Input type="password" value={editing.hmac_secret ?? ""} onChange={(e) => setEditing({ ...editing, hmac_secret: e.target.value })} />
            </Field>
            <div className="flex items-center gap-2">
