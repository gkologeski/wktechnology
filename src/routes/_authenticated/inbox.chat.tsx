...
function LiveChatInbox() {
  ...
  const sessionsQ = useQuery({ queryKey: ["chat-sessions"], queryFn: () => listFn(), refetchInterval: 10000 });
  const messagesQ = useQuery({
    queryKey: ["chat-messages", selected],
    queryFn: () => msgsFn({ data: { session_id: selected! } }),
    enabled: !!selected, refetchInterval: 3000,
  });

  useEffect(() => {
    const ch = supabase.channel("live-chat-inbox")
  ...
  const send = useMutation({
    ...
    onSuccess: () => { setDraft(""); qc.invalidateQueries({ queryKey: ["chat-messages", selected] }); },
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-[300px_1fr] gap-3 h-[calc(100vh-12rem)]">
        <Card className="overflow-hidden flex flex-col">
          <div className="p-2 border-b text-xs text-muted-foreground">{sessions.length} sessão(ões)</div>
          <ScrollArea className="flex-1">
            {sessions.length === 0 && <div className="p-4 text-sm text-muted-foreground">Nenhuma sessão ainda.</div>}
            {sessions.map((s) => (
              <button key={s.id} onClick={() => setSelected(s.id)}
                className={`w-full text-left p-3 border-b hover:bg-muted/50 ${selected === s.id ? "bg-muted" : ""}`}>
                <div className="flex items-center gap-2 text-sm font-medium">
                  <span className="truncate flex-1">{s.visitor_name || s.visitor_email || "Visitante anônimo"}</span>
        ...
        <Card className="overflow-hidden flex flex-col">
          {!current ? (
            <div className="flex-1 grid place-items-center text-sm text-muted-foreground">Selecione uma sessão</div>
          ) : (
            <>
              <div className="p-3 border-b flex items-center justify-between gap-2">
                <div className="min-w-0">
                  <div className="text-sm font-medium truncate">{current.visitor_name || "Visitante"}</div>
                  <div className="text-xs text-muted-foreground truncate">
                    {current.visitor_email || "—"}{current.visitor_url ? ` · ${current.visitor_url}` : ""}
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  {messages.map((m) => (
                    <div key={m.id} className={`flex ${m.direction === "inbound" ? "justify-start" : "justify-end"}`}>
                      <div className={`rounded-2xl px-3 py-2 max-w-[75%] text-sm ${m.direction === "inbound" ? "bg-muted" : "bg-primary text-primary-foreground"}`}>
                        {m.body}
