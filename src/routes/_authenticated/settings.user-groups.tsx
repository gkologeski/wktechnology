...
import {
  listUserGroups, createUserGroup, updateUserGroup, deleteUserGroup, setGroupMembers,
...
type Group = { id: string; name: string; color: string | null; description: string | null; member_ids: string[] };
...
function UserGroupsPage() {
  ...
  const [draft, setDraft] = useState<{ name: string; color: string; description: string }>({ name: "", color: COLORS[0], description: "" });
  const [memberDraft, setMemberDraft] = useState<string[]>([]);

  const openCreate = () => { setDraft({ name: "", color: COLORS[0], description: "" }); setMemberDraft([]); setCreating(true); };
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      if (editing) {
        await updateFn({ data: { id: editing.id, name: draft.name, color: draft.color, description: draft.description || null } });
        await membersFn({ data: { group_id: editing.id, user_ids: memberDraft } });
      } else {
        const res = await createFn({ data: { name: draft.name, color: draft.color, description: draft.description || null } });
        if (memberDraft.length) await membersFn({ data: { group_id: res.id, user_ids: memberDraft } });
      }
    },
    onSuccess: () => {
  ...
  const removeMutation = useMutation({
    ...
    onSuccess: () => { toast.success("Equipe removida"); qc.invalidateQueries({ queryKey: ["user-groups"] }); },
    onError: (e) => toast.error(e instanceof Error ? e.message : "Erro"),
  });
  ...
  const nameById = useMemo(() => new Map(team.map((m) => [m.user_id, m.full_name || m.email || m.user_id])), [team]);
  ...
  return (
    <div className="space-y-4">
      <PageHeader
        ...
        actions={<Button onClick={openCreate}><Plus className="h-4 w-4 mr-2" />Nova equipe</Button>}
      />

      <Card>
        <CardHeader><CardTitle className="text-base">Equipes do workspace</CardTitle></CardHeader>
        <CardContent>
          {groupsQ.isLoading ? (
              ...
              {groups.map((g) => (
                <div key={g.id} className="flex items-start justify-between py-3 gap-3">
                  <div className="flex items-start gap-3 min-w-0">
                    <div className="h-9 w-9 rounded-md flex items-center justify-center text-white shrink-0" style={{ background: g.color ?? "#6b7280" }}>
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="mt-1 flex flex-wrap gap-1">
                        {g.member_ids.length === 0 ? (
                          <Badge variant="outline" className="text-xs">sem membros</Badge>
                        ) : g.member_ids.slice(0, 6).map((uid) => (
                          <Badge key={uid} variant="secondary" className="text-xs">{nameById.get(uid) ?? uid.slice(0, 6)}</Badge>
                        ))}
                        {g.member_ids.length > 6 && <Badge variant="outline" className="text-xs">+{g.member_ids.length - 6}</Badge>}
                      </div>
                    </div>
                  </div>
                  <div className="flex items-center gap-1 shrink-0">
                    <Button size="icon" variant="ghost" onClick={() => openEdit(g)}><Pencil className="h-4 w-4" /></Button>
                    <Button size="icon" variant="ghost" onClick={() => { if (confirm(`Remover equipe "${g.name}"?`)) removeMutation.mutate(g.id); }}>
        ...
        <DialogContent className="max-w-lg">
          ...
          <div className="space-y-3 py-2">
            <div className="space-y-1.5">
              ...
              <Input id="g-name" value={draft.name} onChange={(e) => setDraft({ ...draft, name: e.target.value })} placeholder="Ex: Vendas Inbound" />
            </div>
            <div className="space-y-1.5">
              <Label>Cor</Label>
            ...
            <div className="space-y-1.5">
              ...
              <Textarea id="g-desc" rows={2} value={draft.description} onChange={(e) => setDraft({ ...draft, description: e.target.value })} />
            </div>
            <div className="space-y-1.5">
              <Label>Membros ({memberDraft.length})</Label>
              <div className="border rounded-md max-h-64 overflow-auto divide-y">
                {team.length === 0 && <div className="p-3 text-sm text-muted-foreground">Nenhum usuário no workspace.</div>}
                {team.map((m) => (
                  <label key={m.user_id} className="flex items-center gap-3 px-3 py-2 cursor-pointer hover:bg-muted/50">
                    <Checkbox checked={memberDraft.includes(m.user_id)} onCheckedChange={() => toggleMember(m.user_id)} />
          ...
          <DialogFooter>
            ...
            <Button onClick={() => saveMutation.mutate()} disabled={!draft.name.trim() || saveMutation.isPending}>
