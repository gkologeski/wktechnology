...
import { listOnbPlans, ONB_PLAN_STATUS_LABELS, type OnbPlanStatus } from "@/lib/people/onboarding.functions";
...
export const Route = createFileRoute("/_authenticated/people/offboarding")({
  head: () => ({
    meta: [
      ...
      { property: "og:description", content: "Acompanhe o encerramento de contratos e revogações de acesso." },
    ],
  }),
...
function OffboardingListPage() {
  ...
  return (
    <div className="container max-w-6xl mx-auto p-6 space-y-4">
      <PageHeader title="Offboarding" description="Desligamentos em andamento e checklist de compliance." />
              ...
              <TableRow>
                <TableCell colSpan={6} className="text-center py-12">
                  <div className="flex flex-col items-center gap-2">
                    ...
                    <div className="text-xs text-muted-foreground">Inicie um plano na ficha da pessoa (aba Onboarding · Desligamento).</div>
                  </div>
                </TableCell>
              </TableRow>
            ) : (
              rows.map((p) => {
                ...
                return (
                  <TableRow key={p.id} className="hover:bg-muted/40">
                    <TableCell>
                      <Link to="/people/$id" params={{ id: p.person_id }} className="font-medium hover:underline">
                        {p.person_name ?? "—"}
                      </Link>
                    </TableCell>
                    <TableCell><Badge className={STATUS_TONE[p.status]} variant="outline">{ONB_PLAN_STATUS_LABELS[p.status]}</Badge></TableCell>
                    ...
                    <TableCell className="text-sm text-muted-foreground">{p.started_at ? new Date(p.started_at).toLocaleDateString("pt-BR") : "—"}</TableCell>
                    <TableCell className="text-sm text-muted-foreground">{p.target_completion_date ?? "—"}</TableCell>
                    <TableCell className="text-right">
                      <Link to="/people/$id" params={{ id: p.person_id }} className="text-sm text-primary hover:underline">Abrir</Link>
                    </TableCell>
                  </TableRow>
                );
              })
