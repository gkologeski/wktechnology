...
function CustomObjectsPage() {
  ...
  const [form, setForm] = useState<{ name: string; slug: string; schema: CustomObjectField[] }>({
    name: "",
    slug: "",
    schema: [],
  });
  ...
  const load = async () => {
    const r = await listObj({});
    setObjs(r.objects as Obj[]);
  };
  useEffect(() => {
    void load();
  }, []);
  ...
  const openNew = () => {
    setEdit(null);
    setForm({ name: "", slug: "", schema: [] });
    setOpenObj(true);
  };
  const openEdit = (o: Obj) => {
    setEdit(o);
    setForm({ name: o.name, slug: o.slug, schema: o.schema ?? [] });
    setOpenObj(true);
  };

  const addField = () =>
    setForm((f) => ({ ...f, schema: [...f.schema, { key: "", label: "", type: "text" }] }));
  const updField = (i: number, patch: Partial<CustomObjectField>) =>
    setForm((f) => ({
      ...f,
      schema: f.schema.map((x, idx) => (idx === i ? { ...x, ...patch } : x)),
    }));
  const rmField = (i: number) =>
    setForm((f) => ({ ...f, schema: f.schema.filter((_, idx) => idx !== i) }));
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Crie entidades próprias com schema dinâmico.
          </p>
        </div>
        <Button onClick={openNew}>
          <Plus className="h-4 w-4 mr-2" /> Novo objeto
        </Button>
      </div>
      ...
      <div className="grid gap-3">
            ...
            <CardContent className="pt-6 flex justify-between items-center">
              <div className="space-y-1">
                ...
                <div className="text-xs text-muted-foreground">
                  /{o.slug} · {(o.schema ?? []).length} campos
                </div>
              </div>
              <div className="flex gap-2">
                <Button variant="outline" size="sm" onClick={() => openRecords(o)}>
                  <Eye className="h-4 w-4 mr-1" /> Registros
                </Button>
        ...
        {objs.length === 0 && (
          <p className="text-sm text-muted-foreground">Nenhum objeto custom.</p>
        )}
      </div>
      ...
      <Dialog open={openObj} onOpenChange={setOpenObj}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>{edit ? "Editar objeto" : "Novo objeto"}</DialogTitle>
          </DialogHeader>
              ...
              <div className="space-y-2 mt-2">
                {form.schema.map((f, i) => (
                  <div key={i} className="grid grid-cols-12 gap-2 items-start">
                    ...
                    <Select
                      value={f.type}
                      onValueChange={(v) => updField(i, { type: v as CustomObjectField["type"] })}
                    >
      ...
      <Sheet open={!!active} onOpenChange={(v) => !v && setActive(null)}>
        <SheetContent className="w-full sm:max-w-2xl">
          <SheetHeader>
            <SheetTitle>{active?.name} — registros</SheetTitle>
          </SheetHeader>
          <div className="mt-4 space-y-3">
            ...
            <div className="space-y-2">
              {records.map((r) => (
                <div
                  key={r.id}
                  className="border rounded-md p-3 text-sm flex justify-between items-start gap-2"
                >
                  ...
                  <div className="flex gap-1">
                    ...
                    <Button
                      ...
                      size="sm"
                      onClick={async () => {
                        await delRec({ data: { id: r.id } });
                        if (active) {
                          const x = await listRec({ data: { object_id: active.id } });
                          setRecords(x.records as Rec[]);
                        }
                      }}
                    >
                      <Trash2 className="h-4 w-4" />
          ...
          <Dialog open={recOpen} onOpenChange={setRecOpen}>
            <DialogContent>
              <DialogHeader>
                <DialogTitle>{recEdit ? "Editar registro" : "Novo registro"}</DialogTitle>
              </DialogHeader>
              <div className="space-y-3 max-h-[60vh] overflow-auto">
                {(active?.schema ?? []).map((f) => (
                  <div key={f.key}>
                    ...
                    {f.type === "select" ? (
                      <Select
                        ...
                        onValueChange={(v) => setRecForm({ ...recForm, [f.key]: v })}
                      >
                        <SelectTrigger>
