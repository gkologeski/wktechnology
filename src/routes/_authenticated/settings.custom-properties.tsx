...
import {
  listCustomProperties, upsertCustomProperty, deleteCustomProperty,
  CUSTOM_ENTITIES, CUSTOM_TYPES, CUSTOM_TYPE_LABELS, CUSTOM_ENTITY_LABELS,
  type CustomEntity, type CustomType,
...
function CustomPropsPage() {
  ...
  const openNew = () => { setEditing({ entity, type: "text", options: [], enabled: true, required: false, position: 0 }); setOpen(true); };
  ...
  const handleDelete = async (r: Row) => {
    if (!confirm(`Remover a propriedade "${r.label}"? Os valores já gravados nos registros não serão excluídos.`)) return;
    try { await delFn({ data: { id: r.id } }); await refresh(); toast.success("Removida"); }
    catch (e) { toast.error(e instanceof Error ? e.message : "Erro"); }
  };
      ...
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Crie campos próprios para leads, contatos, empresas e negócios. Aparecem na ficha do registro.
          </p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <Button onClick={openNew}><Plus className="h-4 w-4 mr-2" />Nova propriedade</Button>
        </div>
      </div>
...
function PropertyDialog({
...
}: { editing: Partial<Row> | null; onSave: (f: Partial<Row>) => void | Promise<void>; onCancel: () => void }) {
  ...
  return (
    <DialogContent>
      <DialogHeader><DialogTitle>{editing.id ? "Editar propriedade" : "Nova propriedade"}</DialogTitle></DialogHeader>
      <div className="space-y-3">
        <div className="grid grid-cols-2 gap-2">
          <div className="space-y-1">
            <Label>Entidade</Label>
            <Select value={(form.entity as string) ?? "leads"} onValueChange={(v) => setForm((f) => ({ ...f, entity: v as CustomEntity }))}>
          ...
          <div className="space-y-1">
            <Label>Tipo</Label>
            <Select value={(form.type as string) ?? "text"} onValueChange={(v) => setForm((f) => ({ ...f, type: v as CustomType }))}>
              <SelectTrigger><SelectValue /></SelectTrigger>
              <SelectContent>
                {CUSTOM_TYPES.map((t) => <SelectItem key={t} value={t}>{CUSTOM_TYPE_LABELS[t]}</SelectItem>)}
              </SelectContent>
            </Select>
          </div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-2 items-center">
          <div className="space-y-1">
            <Label>Ordem</Label>
            <Input type="number" value={form.position ?? 0}
      ...
      <DialogFooter>
        ...
        <Button onClick={() => onSave(form)} disabled={!form.label || !form.key}>Salvar</Button>
      </DialogFooter>
    </DialogContent>
  );
}
