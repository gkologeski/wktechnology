...
import {
  listSequences, saveSequence, deleteSequence, listEnrollments,
  updateEnrollmentStatus, triggerSequencesTickNow,
  type SequenceListItem, type EnrollmentListItem,
} from "@/lib/sequences.functions";
import { SequenceBuilder, EMPTY_DRAFT, type SequenceDraft } from "@/components/sequences/sequence-builder";
...
function SequencesPage() {
  ...
  const handleDelete = async (id: string) => {
    ...
    try { await deleteFn({ data: { id } }); toast.success("Removida"); refresh(); }
    catch (e) { toast.error(e instanceof Error ? e.message : "Erro"); }
  };
  const handleToggle = async (row: SequenceListItem, enabled: boolean) => {
    try {
      await saveFn({ data: { id: row.id, name: row.name, entity: row.entity, enabled, steps: row.steps } });
      refresh();
    } catch (e) { toast.error(e instanceof Error ? e.message : "Erro"); }
  };
  const handleTick = async () => {
    try { const r = await tickFn(); toast.success(`Processados: ${r.processed} enrollment(s)`); refresh(); }
    catch (e) { toast.error(e instanceof Error ? e.message : "Erro"); }
  };
  const handleEnrStatus = async (id: string, status: "active" | "paused" | "removed") => {
    try { await enrStatusFn({ data: { id, status } }); refresh(); }
    catch (e) { toast.error(e instanceof Error ? e.message : "Erro"); }
  };
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Cadências automáticas de tarefas e e-mails.</p>
        </div>
        <div className="flex gap-2">
          <Button variant="outline" onClick={handleTick}><Zap className="h-4 w-4 mr-1" /> Rodar agora</Button>
          <Button onClick={() => setDraft({ ...EMPTY_DRAFT })}><Plus className="h-4 w-4 mr-1" /> Nova sequência</Button>
        </div>
      </div>
              ...
              <CardHeader className="flex-row items-start justify-between space-y-0">
                <div>
                  <CardTitle className="text-base flex items-center gap-2">
                    {row.name}
                    {row.active_enrollments > 0 && <Badge variant="secondary">{row.active_enrollments} ativas</Badge>}
                  </CardTitle>
                  <p className="text-xs text-muted-foreground mt-1">
                    {ENTITY_LABELS[row.entity]} · {row.steps?.length ?? 0} passo(s) · {row.completed_enrollments} concluída(s)
                  </p>
                </div>
                <div className="flex items-center gap-1">
                  <Switch checked={row.enabled} onCheckedChange={(v) => handleToggle(row, v)} />
                  <Button variant="ghost" size="icon" onClick={() => setDraft({
                    id: row.id, name: row.name, entity: row.entity, enabled: row.enabled, steps: row.steps,
                  })}>
              ...
              <ul className="divide-y">
                {enrollments.map((e) => (
                  <li key={e.id} className="flex items-center gap-3 p-3 text-sm">
                    ...
                    {e.status === "active" && (
                      <Button size="icon" variant="ghost" title="Pausar" onClick={() => handleEnrStatus(e.id, "paused")}>
                        <Pause className="h-4 w-4" />
                      </Button>
                    )}
                    {e.status === "paused" && (
                      <Button size="icon" variant="ghost" title="Retomar" onClick={() => handleEnrStatus(e.id, "active")}>
                        <Play className="h-4 w-4" />
                      </Button>
                    )}
                    {e.status !== "removed" && e.status !== "completed" && (
                      <Button size="icon" variant="ghost" title="Remover" onClick={() => handleEnrStatus(e.id, "removed")}>
                        <Trash2 className="h-4 w-4" />
                      </Button>
                    )}
                  </li>
