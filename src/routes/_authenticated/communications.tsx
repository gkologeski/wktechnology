...
function CommunicationsPage() {
  return (
    <EntityList<Activity>
      ...
      lockedFilters={[
        { type: "condition", field: "type", op: "in", value: COMM_TYPES as unknown as string[] },
      ]}
      ...
      columns={[
        {
          key: "type",
          label: "Tipo",
          render: (r) => ACTIVITY_TYPES.find((t) => t.value === r.type)?.label ?? r.type,
        },
        { key: "subject", label: "Assunto", render: (r) => r.subject || "(sem assunto)" },
        {
          key: "outcome",
          label: "Resultado",
          render: (r) => r.outcome ?? r.meeting_outcome ?? r.disposition ?? "—",
        },
        {
          key: "duration_ms",
          label: "Duração",
          render: (r) => (r.duration_ms ? `${Math.round(r.duration_ms / 1000 / 60)} min` : "—"),
        },
      ...
      fields={[
        {
          ...
          options: COMM_TYPES.map((t) => ({
            value: t,
            label: ACTIVITY_TYPES.find((a) => a.value === t)!.label,
          })),
        },
