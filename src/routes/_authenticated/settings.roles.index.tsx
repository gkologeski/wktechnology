...
import {
  listAccessProfiles, createAccessProfile, deleteAccessProfile,
...
function RolesPage() {
  ...
  const refresh = async () => {
    ...
    try {
      ...
      setProfiles(p); setAssignments(a);
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      setCreateOpen(false); setNewName(""); setNewDesc(""); setCopyFrom("none"); setNewRole("member");
  ...
  const handleDuplicate = async (p: ProfileRow) => {
    try {
      const res = await createFn({ data: { name: `${p.name} (cópia)`, description: p.description ?? undefined, base_role: p.base_role, copy_from: p.id } });
  ...
  const handleDelete = async (id: string) => {
    try { await deleteFn({ data: { id } }); toast.success("Perfil excluído"); refresh(); }
    catch (e) { toast.error(e instanceof Error ? e.message : "Erro"); }
  };
  ...
  const handleAssign = async (user_id: string, profile_id: string) => {
    try {
      ...
      setAssignments((rs) => rs.map((r) => r.user_id === user_id ? { ...r, access_profile_id: profile_id } : r));
    } catch (e) { toast.error(e instanceof Error ? e.message : "Erro"); }
  };
  ...
  return (
    <div className="space-y-4">
          ...
          <DialogContent>
            ...
            <div className="space-y-3">
              <div>
                ...
                <Input value={newName} onChange={(e) => setNewName(e.target.value)} placeholder="ex: Comercial Pleno" />
              </div>
              <div>
                <label className="text-xs font-medium">Descrição</label>
                <Textarea value={newDesc} onChange={(e) => setNewDesc(e.target.value)} placeholder="Para que serve este perfil…" rows={2} />
              </div>
      ...
      <Card>
        ...
        <CardContent className="space-y-2">
          ...
          {!loading && profiles.map((p) => (
            <div key={p.id} className="grid grid-cols-[1fr_auto_auto_auto] gap-3 items-center py-2 border-b last:border-0">
              <div className="space-y-0.5 min-w-0">
                <div className="flex items-center gap-2">
                  <Shield className="h-3.5 w-3.5 text-muted-foreground" />
                  <span className="text-sm font-medium">{p.name}</span>
                  {p.is_system && <Badge variant="secondary" className="text-[10px]">Sistema</Badge>}
                </div>
                {p.description && <p className="text-xs text-muted-foreground truncate">{p.description}</p>}
              </div>
              <Badge variant="outline" className="text-[10px]">{p.user_count} usuário(s)</Badge>
              <Link to="/settings/roles/$roleId" params={{ roleId: p.id }}>
                <Button size="sm" variant="outline"><Pencil className="h-3.5 w-3.5 mr-1.5" />Editar</Button>
              </Link>
                ...
                {!p.is_system && (
                  <AlertDialog>
                    <AlertDialogTrigger asChild>
                      <Button size="icon" variant="ghost" title="Excluir"><Trash2 className="h-3.5 w-3.5" /></Button>
                    </AlertDialogTrigger>
                    <AlertDialogContent>
                      <AlertDialogHeader>
                        <AlertDialogTitle>Excluir "{p.name}"?</AlertDialogTitle>
                        <AlertDialogDescription>
                          Esta ação não pode ser desfeita. Se houver usuários atribuídos, mova-os antes.
                        </AlertDialogDescription>
                      </AlertDialogHeader>
                      <AlertDialogFooter>
      ...
      <Card>
        <CardHeader><CardTitle className="text-base">Atribuição por usuário</CardTitle></CardHeader>
        <CardContent className="space-y-2">
          {!loading && assignments.map((u) => (
            <div key={u.user_id} className="grid grid-cols-[1fr_220px] gap-3 items-center py-2 border-b last:border-0">
              <div className="flex items-center gap-2">
              ...
              {u.is_owner ? (
                ...
                <Select value={u.access_profile_id ?? ""} onValueChange={(v) => handleAssign(u.user_id, v)}>
