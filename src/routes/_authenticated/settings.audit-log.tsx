      ...
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Registro automático de criação, alteração e exclusão de leads, contatos, empresas e negócios.
          </p>
        </div>
            ...
            <div className="space-y-1">
              ...
              <Select value={entity} onValueChange={setEntity}>
                ...
                <SelectContent>
                  ...
                  {(Object.keys(AUDIT_ENTITY_LABELS) as Array<keyof typeof AUDIT_ENTITY_LABELS>).map((k) =>
                    <SelectItem key={k} value={k}>{AUDIT_ENTITY_LABELS[k]}</SelectItem>)}
                </SelectContent>
              </Select>
            </div>
            <div className="space-y-1">
              ...
              <Select value={action} onValueChange={setAction}>
                ...
                <SelectContent>
                  <SelectItem value="all">Todas</SelectItem>
                  {(Object.keys(AUDIT_ACTION_LABELS) as Array<keyof typeof AUDIT_ACTION_LABELS>).map((k) =>
                    <SelectItem key={k} value={k}>{AUDIT_ACTION_LABELS[k]}</SelectItem>)}
                </SelectContent>
              </Select>
            </div>
            ...
            <div className="text-sm">
              ...
              {rows.map((r) => (
                <div key={r.id}
                  ...
                  <span>{AUDIT_ENTITY_LABELS[r.entity as keyof typeof AUDIT_ENTITY_LABELS] ?? r.entity}</span>
                  ...
                  <span className="truncate text-xs text-muted-foreground">
                    {r.action === "updated"
                      ? (r.changed_keys.length ? r.changed_keys.slice(0, 5).join(", ") + (r.changed_keys.length > 5 ? "…" : "") : "—")
                      : (r.entity_id ?? "")}
                  </span>
                  <span className="text-xs truncate">{r.actor_name || r.actor_email || (r.actor_user_id ? r.actor_user_id.slice(0, 8) : "sistema")}</span>
                  <Button variant="ghost" size="icon" onClick={() => setDetail(r)} aria-label="Detalhes">
        ...
        {row && (
          <div className="space-y-3 text-sm">
            <div className="grid grid-cols-2 gap-2">
              ...
              <div><span className="text-muted-foreground">Ação:</span> {row.action}</div>
            </div>

            {row.action === "updated" && (
              <div>
                <div className="font-medium mb-1">Mudanças</div>
                {diff.length === 0 && <p className="text-xs text-muted-foreground">Nenhuma diferença relevante.</p>}
                {diff.length > 0 && (
                  <div className="border rounded">
                    ...
                    {diff.map((d) => (
                      <div key={d.key} className="grid grid-cols-[160px_1fr_1fr] gap-2 px-2 py-1 border-b last:border-0 text-xs">
