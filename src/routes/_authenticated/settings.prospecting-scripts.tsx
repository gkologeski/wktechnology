...
import { listScripts, upsertScript, deleteScript, type ProspectingScript } from "@/lib/prospecting-scripts.functions";
...
function ScriptsPage() {
  ...
  const testVoice = async () => {
    if (!edit?.voice_id || !edit?.first_message) {
      toast.error("Defina uma voz e a mensagem de abertura primeiro"); return;
    }
    setPreviewing(true);
    try {
    ...
    <div className="space-y-4">
      <div className="flex justify-between items-start">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Crie variações de roteiro para testar em campanhas A/B.</p>
        </div>
        <Button onClick={() => { setEdit({ voice_provider: "elevenlabs" }); setOpen(true); }}>
          <Plus className="h-4 w-4 mr-2" />Novo script
        </Button>
      </div>

      <Card><CardContent className="pt-4">
        {rows.length === 0 && <p className="text-sm text-muted-foreground">Nenhum script ainda.</p>}
        <div className="divide-y">
          {rows.map((r) => (
            <div key={r.id} className="py-2 flex items-center justify-between gap-2">
              <button className="text-left flex-1 min-w-0" onClick={() => { setEdit(r); setOpen(true); }}>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-2xl">
          ...
          <div className="space-y-2">
            <div><Label>Nome</Label><Input value={edit?.name ?? ""} onChange={(e) => setEdit((f) => ({ ...f, name: e.target.value }))} /></div>
            <div><Label>Objetivo</Label><Input value={edit?.objective ?? ""} onChange={(e) => setEdit((f) => ({ ...f, objective: e.target.value }))} placeholder="Qualificar, agendar reunião..." /></div>
            <div><Label>Voz (ElevenLabs voice_id)</Label>
              <div className="flex gap-2">
                <Input className="flex-1" value={edit?.voice_id ?? ""} onChange={(e) => setEdit((f) => ({ ...f, voice_id: e.target.value }))} placeholder={CURATED_VOICES[0].id} />
              ...
              <p className="text-xs text-muted-foreground mt-1">Deixe vazio para usar a voz padrão do workspace.</p>
            </div>
            <div><Label>Mensagem de abertura</Label>
              <Textarea rows={3} value={edit?.first_message ?? ""} onChange={(e) => setEdit((f) => ({ ...f, first_message: e.target.value }))} placeholder="Oi {{lead.name}}, aqui é a Sara da..." />
            </div>
            <div><Label>Prompt do sistema</Label>
              <Textarea rows={8} value={edit?.system_prompt ?? ""} onChange={(e) => setEdit((f) => ({ ...f, system_prompt: e.target.value }))}
                placeholder="Você é um SDR ligando para {{lead.name}} da empresa {{lead.company}}. Seu objetivo é..." />
            </div>
            <p className="text-xs text-muted-foreground">Variáveis disponíveis: <code>{`{{lead.name}}`}</code>, <code>{`{{lead.company}}`}</code>.</p>
          </div>
          <DialogFooter>
