...
import {
  listProposals, createProposal, deleteProposal,
...
const STATUS_LABEL: Record<string, string> = {
  ...
  sent: "Enviada", accepted: "Aceita", rejected: "Recusada",
  expired: "Expirada", canceled: "Cancelada",
};
const STATUS_VARIANT: Record<string, "default" | "secondary" | "destructive" | "outline"> = {
  draft: "outline", in_review: "secondary", approved: "secondary",
  sent: "default", accepted: "default", rejected: "destructive",
  expired: "outline", canceled: "outline",
};
...
function ProposalsPage() {
  ...
  return (
    <div className="space-y-4 p-6">
      ...
      <Card>
        <CardHeader><CardTitle>Suas propostas</CardTitle></CardHeader>
        <CardContent className="space-y-2">
          {(data ?? []).length === 0 && <p className="text-sm text-muted-foreground">Nenhuma proposta ainda.</p>}
          {(data ?? []).map((p) => (
            <div key={p.id} className="flex items-center justify-between rounded-md border p-3">
              <div className="flex items-center gap-3">
                <FileText className="h-4 w-4 text-muted-foreground" />
                <div>
                  <Link to="/proposals/$id" params={{ id: p.id }} className="font-medium hover:underline">{p.title}</Link>
                  <div className="text-xs text-muted-foreground">
                    v{p.version} · {p.total_amount ? new Intl.NumberFormat("pt-BR", { style: "currency", currency: p.currency }).format(Number(p.total_amount)) : "sem valor"}
                  </div>
                </div>
              </div>
              <div className="flex items-center gap-2">
                <Badge variant={STATUS_VARIANT[p.status] ?? "outline"}>{STATUS_LABEL[p.status] ?? p.status}</Badge>
                <Button size="icon" variant="ghost" onClick={() => { if (confirm("Remover proposta?")) delM.mutate(p.id); }}>
