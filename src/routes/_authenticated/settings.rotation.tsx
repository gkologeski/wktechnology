...
import {
  listRotationRules, saveRotationRule, deleteRotationRule, listWorkspaceMembers,
} from "@/lib/rotation.functions";
import { ROT_ENTITY_LABELS, STRATEGY_LABELS, type RotationAssignee, type RotationEntity, type RotationStrategy } from "@/lib/rotation/types";
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Defina regras de rotação para distribuir Leads e Negócios entre o seu time. Use no Workflow como ação "Distribuir via regra".
          </p>
        </div>
            ...
            <div className="space-y-1">
              ...
              <div className="flex items-center gap-2 pt-1">
                ...
                <button
                  ...
                  onClick={() => { navigator.clipboard.writeText(r.id as string); toast.success("ID copiado"); }}>
      ...
      <Sheet open={!!draft} onOpenChange={(o) => !o && setDraft(null)}>
        <SheetContent className="w-full sm:max-w-xl overflow-y-auto">
          <SheetHeader><SheetTitle>{draft?.id ? "Editar regra" : "Nova regra de distribuição"}</SheetTitle></SheetHeader>
          {draft && (
            <div className="space-y-5 py-4">
              <div className="grid grid-cols-[1fr_auto] gap-3 items-end">
                <div>
                  <Label>Nome</Label>
                  <Input value={draft.name} onChange={(e) => setDraft({ ...draft, name: e.target.value })} placeholder="Ex: Leads do site — comercial" />
                </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <Select value={draft.entity} onValueChange={(v) => setDraft({ ...draft, entity: v as RotationEntity })}>
                    <SelectTrigger><SelectValue /></SelectTrigger>
                    <SelectContent>
                      {(Object.keys(ROT_ENTITY_LABELS) as RotationEntity[]).map((e) => (
                        <SelectItem key={e} value={e}>{ROT_ENTITY_LABELS[e]}</SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
                </div>
                <div>
                  ...
                  <Select value={draft.strategy} onValueChange={(v) => setDraft({ ...draft, strategy: v as RotationStrategy })}>
                    <SelectTrigger><SelectValue /></SelectTrigger>
                    <SelectContent>
              ...
              <section className="rounded-md border p-3 space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <Select onValueChange={(uid) => {
                    ...
                    setDraft({ ...draft, assignees: [...draft.assignees, { user_id: uid, weight: 1 }] });
                  }}>
                    <SelectTrigger className="w-[240px]"><SelectValue placeholder="+ Adicionar membro" /></SelectTrigger>
                    <SelectContent>
                      {members.map((m) => (
                        <SelectItem key={m.user_id} value={m.user_id}>{m.full_name}{m.is_owner ? " (admin)" : ""}</SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
                </div>
                {draft.assignees.length === 0 && (
                  <p className="text-xs text-muted-foreground">Adicione pelo menos um membro do time.</p>
                )}
                {draft.assignees.map((a, i) => (
                  <div key={a.user_id} className="grid grid-cols-[1fr_120px_auto] gap-2 items-center">
                    <span className="text-sm">{nameById.get(a.user_id) ?? a.user_id.slice(0, 8)}</span>
                    {draft.strategy === "weighted" ? (
                      <Input type="number" min={1} max={100} value={a.weight}
                        onChange={(e) => setDraft({
                          ...draft,
                          assignees: draft.assignees.map((x, idx) => idx === i ? { ...x, weight: Math.max(1, Number(e.target.value) || 1) } : x),
                        })} />
                    ) : (<span className="text-xs text-muted-foreground">peso ignorado</span>)}
                    <Button variant="ghost" size="icon" onClick={() => setDraft({
                      ...draft, assignees: draft.assignees.filter((_, idx) => idx !== i),
