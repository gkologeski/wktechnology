...
const ENTITY_FIELDS: Record<SegmentEntity, { name: string; label: string; type?: string; options?: { value: string; label: string }[] }[]> = {
  leads: [
  ...
  contacts: [
    ...
    { name: "score", label: "Score", type: "number" },
    { name: "marketing_status", label: "Status de marketing", type: "select", options: [
  ...
  deals: [
    ...
    { name: "stage", label: "Etapa", type: "select", options: [
      { value: "new", label: "Novo" },
      { value: "qualified", label: "Qualificado" },
        ...
        <Table>
          ...
          <TableBody>
            ...
            {q.data && q.data.length === 0 && (
              <TableRow><TableCell colSpan={7} className="text-center text-muted-foreground py-6">Nenhuma lista criada ainda.</TableCell></TableRow>
            )}
            {q.data?.map((s) => (
              <TableRow key={s.id}>
                ...
                <TableCell className="text-muted-foreground text-sm">{formatDateTime(s.last_refreshed_at)}</TableCell>
                <TableCell>
                  <Badge variant={s.enabled ? "outline" : "secondary"}>{s.enabled ? "Ativa" : "Pausada"}</Badge>
                </TableCell>
                <TableCell>
                  <div className="flex items-center justify-end gap-1">
                    <Button size="icon" variant="ghost" className="h-8 w-8" onClick={() => setViewingId(s.id)} title="Ver membros">
                      <Users className="h-4 w-4" />
                    </Button>
                    {s.kind === "dynamic" && (
                      <Button size="icon" variant="ghost" className="h-8 w-8" disabled={refreshMut.isPending} onClick={() => refreshMut.mutate(s.id)} title="Atualizar agora">
                        <RefreshCw className={`h-4 w-4 ${refreshMut.isPending ? "animate-spin" : ""}`} />
                      </Button>
                    )}
                    <Button size="icon" variant="ghost" className="h-8 w-8" onClick={() => { setEditing(s); setOpen(true); }} title="Editar">
                      <Pencil className="h-4 w-4" />
                    </Button>
                    <Button size="icon" variant="ghost" className="h-8 w-8 text-destructive" onClick={() => removeMut.mutate(s.id)} title="Remover">
...
function SegmentDialog({
  ...
  const [entity, setEntity] = useState<SegmentEntity>((editing?.entity as SegmentEntity) ?? "contacts");
      ...
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="max-w-lg">
          ...
          <div className="space-y-3">
            <div>
              <Label>Nome</Label>
              <Input value={name} onChange={(e) => setName(e.target.value)} placeholder="Ex.: Leads quentes SP" />
            </div>
            ...
            {kind === "dynamic" && (
              <>
                <div>
                  <Label>Filtros</Label>
                  <Button type="button" variant="outline" className="w-full justify-start" onClick={() => setFilterOpen(true)}>
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div className="flex items-end gap-2">
                    ...
                    <Label htmlFor="enabled" className="mb-2">Ativa</Label>
                  </div>
                </div>
              </>
            )}
