...
import {
  listWabas, listTemplates, syncTemplates, submitTemplate,
...
function TemplatesPage() {
  ...
  return (
    <div className="container mx-auto max-w-5xl py-8 space-y-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base flex items-center gap-2"><Plus className="size-4" /> Novo template</CardTitle>
          <CardDescription>O nome precisa ser único, snake_case (a-z, 0-9, _). A aprovação leva 1–24h.</CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={onSubmit} className="grid gap-3 sm:grid-cols-2">
            <div className="space-y-1">
              ...
              <Select value={wabaId} onValueChange={setWabaId}>
                ...
                <SelectContent>
                  {wabas.map((w) => (
                    <SelectItem key={w.id} value={w.id}>{w.business_name || w.waba_id}</SelectItem>
                  ))}
                </SelectContent>
              </Select>
            </div>
            ...
            <div className="space-y-1">
              <Label>Nome</Label>
              <Input value={name} onChange={(e) => setName(e.target.value.toLowerCase().replace(/[^a-z0-9_]/g, "_"))} placeholder="lembrete_reuniao" />
            </div>
            <div className="space-y-1">
              <Label>Idioma</Label>
            ...
            <div className="space-y-1 sm:col-span-2">
              ...
              <Input value={headerText} onChange={(e) => setHeaderText(e.target.value)} placeholder="Confirmação de reunião" />
            </div>
            <div className="space-y-1 sm:col-span-2">
              <Label>Body *</Label>
              <Textarea
            ...
            <div className="space-y-1 sm:col-span-2">
              ...
              <Input value={footerText} onChange={(e) => setFooterText(e.target.value)} placeholder="Equipe Atendimento" />
            </div>
      ...
      <Card>
        ...
        <CardContent>
          {loading ? <Loader2 className="size-4 animate-spin" /> : templates.length === 0 ? (
              ...
              {templates.map((t) => (
                <div key={t.id} className="border rounded-lg p-3 flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    ...
                    {t.rejection_reason && (
                      <div className="text-xs text-destructive mt-1">Recusa: {t.rejection_reason}</div>
                    )}
                  </div>
