    ...
    <div className="p-6 space-y-6 max-w-3xl">
      <div>
        <h1 className="text-2xl font-semibold flex items-center gap-2">
          <ShieldCheck className="h-5 w-5" /> Política de Acesso
        </h1>
        <p className="text-sm text-muted-foreground">
          Controle quem pode acessar este workspace e em que condições.
        </p>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Quando ativada, apenas IPs/faixas listados podem se autenticar. Use formato exato
            (1.2.3.4) ou prefixo /24.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="flex items-center gap-2">
            <Switch
              checked={!!sec.ip_allowlist_enabled}
              onCheckedChange={(v) => save({ ip_allowlist_enabled: v })}
            />
            <Label>Ativar enforcement</Label>
          </div>
          <div className="flex gap-2">
            <Input
              placeholder="Ex.: 200.100.50.10 ou 200.100.50.0/24"
              value={newIp}
          ...
          <div className="space-y-1">
            ...
            {(sec.ip_allowlist ?? []).length === 0 && (
              <p className="text-xs text-muted-foreground">Nenhum IP cadastrado.</p>
            )}
          </div>
        </CardContent>
      </Card>
        ...
        <CardContent>
          <div className="flex items-center gap-3">
            <Input
              ...
              onChange={(e) =>
                setSec({ ...sec, session_timeout_minutes: parseInt(e.target.value || "0", 10) })
              }
              onBlur={(e) =>
                save({ session_timeout_minutes: parseInt(e.target.value || "720", 10) })
              }
            />
        ...
        <CardContent className="space-y-1 text-sm">
          ...
          {accessLog.map((l) => (
            <div key={l.id} className="border rounded p-2 flex justify-between">
              ...
              <Badge variant={l.blocked ? "destructive" : "secondary"}>
