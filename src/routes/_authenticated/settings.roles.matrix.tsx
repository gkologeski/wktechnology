...
} from "@/lib/access-profiles.constants";
    ...
    <div className="space-y-4">
      <div className="flex items-start justify-between gap-3">
        ...
        <Link to="/settings/roles">
          <Button variant="outline" size="sm"><ArrowLeft className="h-4 w-4 mr-1.5" />Voltar</Button>
        </Link>
      </div>

      <Card>
        <CardHeader><CardTitle className="text-base">Perfis especiais (sempre presentes)</CardTitle></CardHeader>
        <CardContent className="grid gap-3 sm:grid-cols-2">
          <div className="rounded-md border p-3 space-y-1">
            ...
            <p className="text-xs text-muted-foreground">
              Bypass total. Único que enxerga Billing/Assinatura e exclusão do workspace.
              Sempre tratado como Admin, ignora limites de perfil.
            </p>
          </div>
          <div className="rounded-md border p-3 space-y-1">
            <div className="flex items-center gap-2">
      ...
      {!isLoading && data && (
        <>
          <Card>
            <CardHeader>
              <CardTitle className="text-base">Escopo por objeto (view · edit · delete · create)</CardTitle>
            </CardHeader>
            <CardContent className="overflow-x-auto">
              <table className="w-full text-sm border-separate border-spacing-0">
                <thead>
                  <tr>
                    <th className="text-left p-2 sticky left-0 bg-background z-10 border-b">Objeto</th>
                    {data.profiles.map((p) => (
                      <th key={p.id} className="text-left p-2 border-b min-w-[180px]">
                        <div className="flex items-center gap-1.5">
                          <Shield className="h-3.5 w-3.5 text-muted-foreground" />
                          <span className="font-medium">{p.name}</span>
                          {p.is_system && <Badge variant="secondary" className="text-[9px] px-1">Sistema</Badge>}
                        </div>
                  ...
                  {ACCESS_OBJECTS.map((obj) => (
                    <tr key={obj.key} className="hover:bg-muted/30">
                      <td className="p-2 sticky left-0 bg-background z-10 border-b font-medium">
                        {obj.label}
                        <div className="text-[10px] text-muted-foreground font-normal">{obj.category}</div>
                      </td>
                      {data.profiles.map((p) => {
                        const perm = data.permissions[p.id]?.[obj.key];
                        if (!perm) return <td key={p.id} className="p-2 border-b text-xs text-muted-foreground">—</td>;
                        return (
                          <td key={p.id} className="p-2 border-b">
                            <div className="flex flex-wrap gap-1">
                              <span className="text-[10px] text-muted-foreground">V:</span><ScopeBadge value={perm.view_scope as Scope} />
                              <span className="text-[10px] text-muted-foreground">E:</span><ScopeBadge value={perm.edit_scope as Scope} />
                              <span className="text-[10px] text-muted-foreground">D:</span><ScopeBadge value={perm.delete_scope as Scope} />
                              {perm.create_enabled && <Badge variant="outline" className="text-[10px]">+criar</Badge>}
                            </div>
                          </td>
                        );
                      })}
          ...
          <Card>
            <CardHeader><CardTitle className="text-base">Ferramentas habilitadas</CardTitle></CardHeader>
            <CardContent className="overflow-x-auto">
              <table className="w-full text-sm border-separate border-spacing-0">
                <thead>
                  <tr>
                    <th className="text-left p-2 sticky left-0 bg-background z-10 border-b">Ferramenta</th>
                    {data.profiles.map((p) => (
