  ...
  const onCreate = async () => {
    ...
    try {
      const r = await create({
        data: { name: name.trim(), scopes: scopes as ("read" | "write")[] },
      });
      setSecret(r.secret);
      setName("");
      setScopes(["read"]);
    ...
    <div className="p-6 space-y-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Use estas chaves para acessar a API pública REST em{" "}
            <code className="text-xs">/api/public/v1/*</code>
          </p>
        </div>
        <Button onClick={() => setOpen(true)}>
          <KeyRound className="h-4 w-4 mr-2" /> Nova chave
        </Button>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Envie sua chave em <code>Authorization: Bearer SUA_CHAVE</code>
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-2 text-sm">
          <div>
            <Badge variant="secondary">GET</Badge> <code>/api/public/v1/contacts</code>
          </div>
          <div>
            <Badge variant="secondary">POST</Badge> <code>/api/public/v1/contacts</code>
          </div>
          <div>
            <Badge variant="secondary">GET</Badge> <code>/api/public/v1/leads</code>
          </div>
          <div>
            <Badge variant="secondary">POST</Badge> <code>/api/public/v1/leads</code>
          </div>
          <div>
            <Badge variant="secondary">GET</Badge> <code>/api/public/v1/deals</code>
          </div>
        </CardContent>
      </Card>
      ...
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{secret ? "Chave criada" : "Nova chave"}</DialogTitle>
            {secret && (
              <DialogDescription>Copie agora — não será exibida novamente.</DialogDescription>
            )}
          </DialogHeader>
          {secret ? (
            <div className="space-y-2">
