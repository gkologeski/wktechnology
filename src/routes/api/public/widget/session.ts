      ...
      POST: async ({ request }) => {
        try {
          ...
          const { data: ws } = await supabaseAdmin
            .from("workspaces").select("id").eq("id", data.workspace_id).maybeSingle();
          if (!ws) return Response.json({ error: "Workspace inválido" }, { status: 404, headers: CORS });
          // Reuse open session for this visitor
          const { data: existing } = await supabaseAdmin
            .from("live_chat_sessions").select("id")
          ...
          const { data: row, error } = await supabaseAdmin.from("live_chat_sessions").insert({
