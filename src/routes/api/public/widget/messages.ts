...
const sendSchema = z.object({
  ...
  visitor_id: z.string().min(8).max(64).regex(/^[a-zA-Z0-9_-]+$/),
...
async function loadSession(sessionId: string, visitorId: string) {
  const { data } = await supabaseAdmin.from("live_chat_sessions")
  ...
  server: {
    handlers: {
      ...
      GET: async ({ request }) => {
        ...
        if (!sessionId || !visitorId) return Response.json({ error: "missing" }, { status: 400, headers: CORS });
        const s = await loadSession(sessionId, visitorId);
        if (!s) return Response.json({ error: "invalid session" }, { status: 403, headers: CORS });
        let q = supabaseAdmin.from("live_chat_messages")
      ...
      POST: async ({ request }) => {
        try {
          ...
          if (!s) return Response.json({ error: "invalid session" }, { status: 403, headers: CORS });
          if (s.status === "closed") return Response.json({ error: "closed" }, { status: 400, headers: CORS });
          const now = new Date().toISOString();
          const { error } = await supabaseAdmin.from("live_chat_messages").insert({
            session_id: data.session_id, owner_id: s.owner_id,
            direction: "inbound", body: data.body,
          });
          if (error) throw error;
          await supabaseAdmin.from("live_chat_sessions")
            .update({ last_message_at: now }).eq("id", data.session_id);
          return Response.json({ ok: true }, { headers: CORS });
        } catch (e) {
          return Response.json({ error: e instanceof Error ? e.message : "erro" }, { status: 400, headers: CORS });
        }
      },
    },
  },
