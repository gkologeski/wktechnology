...
async function handleMessageChange(value: any) {
  ...
  const { data: pn } = await supabaseAdmin
    ...
    .select("workspace_id, display_phone_number")
  ...
  for (const m of value?.messages ?? []) {
    ...
    const convId = await findOrCreateConversation(
      phoneNumberId,
      displayPhone || pn.display_phone_number,
      from,
      workspaceId,
    );
    if (!convId) continue;
  ...
  for (const s of value?.statuses ?? []) {
    ...
    if (s.status === "delivered")
      patch.delivered_at = new Date(Number(s.timestamp) * 1000).toISOString();
...
async function handleQualityUpdate(value: any) {
  ...
  if (!phoneNumberId) return;
  const patch: any = {};
  if (value.current_quality_rating || value.current_quality)
    patch.quality_rating = value.current_quality_rating ?? value.current_quality;
  if (value.current_limit || value.new_limit)
    patch.messaging_limit_tier = value.new_limit ?? value.current_limit;
  if (Object.keys(patch).length) {
    await supabaseAdmin
      .from("wa_phone_numbers")
      .update(patch)
      .eq("phone_number_id", String(value.phone_number_id));
  }
}
