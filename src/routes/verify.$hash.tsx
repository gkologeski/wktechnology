...
function VerifyPage() {
  ...
  const { data, isLoading } = useQuery({ queryKey: ["verify", hash], queryFn: () => verify({ data: { hash } }) });
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            {data?.found ? <ShieldCheck className="h-5 w-5 text-primary" /> : <ShieldAlert className="h-5 w-5 text-destructive" />}
        ...
        <CardContent className="space-y-3">
          ...
          {!isLoading && !data?.found && (
            <p className="text-sm text-destructive">Hash não encontrado. Documento inválido ou não selado.</p>
          )}
          {data?.found && (
