...
function PortalPage() {
  ...
  const mutation = useMutation({
    mutationFn: () => create({ data: { token, subject, description: description || undefined, priority } }),
    onSuccess: () => {
  ...
  if (session.error) {
    return <div className="min-h-screen grid place-items-center text-destructive">{(session.error as Error).message}</div>;
  }
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                Olá, {contact.first_name}{contact.last_name ? ` ${contact.last_name}` : ""}
              </p>
            </div>
          </div>
            ...
            <DialogContent>
              ...
              <div className="space-y-3">
                <div>
                  ...
                  <Input value={subject} onChange={(e) => setSubject(e.target.value)} placeholder="Resuma sua solicitação" />
                </div>
                <div>
                  <label className="text-sm font-medium">Descrição</label>
                  <Textarea rows={5} value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Detalhe o que aconteceu" />
                </div>
                <div>
              ...
              <DialogFooter>
                ...
                <Button onClick={() => mutation.mutate()} disabled={!subject.trim() || mutation.isPending}>
