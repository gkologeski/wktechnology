...
export const listMyWorkspaces = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const workspaces = (members ?? [])
      .map((m) => {
        const w = (m as { workspaces: { id: string; name: string; slug: string; logo_url: string | null; primary_color: string | null; status: string } | null }).workspaces;
        if (!w) return null;
        return { ...w, role: m.role as "admin" | "member" };
      })
      .filter((x): x is { id: string; name: string; slug: string; logo_url: string | null; primary_color: string | null; status: string; role: "admin" | "member" } => !!x);
