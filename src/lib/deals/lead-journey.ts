...
export function normalizeLeadChannel(source: string | null | undefined): LeadChannel {
  ...
  if (has(value, ["prospect", "apollo", "outbound", "cold_call", "cold_email", "linkedin"])) return "prospecting";
  if (has(value, ["paid", "cpc", "ppc", "google_ads", "facebook_ads", "meta_ads", "linkedin_ads"])) return "paid";
  ...
  if (has(value, ["form", "website", "site", "landing", "direct_traffic", "chat"])) return "website";
...
export function resolveJourneyStage(
  ...
  if (lead.status === "contacted" || lead.status === "nurturing") return stages.filter((stage) => (stage.type ?? "open") === "open")[1] ?? stages[0];
  return stages[0];
}
