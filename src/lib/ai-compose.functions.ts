...
function instruction(mode: z.infer<typeof Mode>, channel: z.infer<typeof Channel>): string {
  ...
  switch (mode) {
    case "draft":
      return `Redija uma mensagem nova a partir da intenção descrita. ${tone}`;
    case "improve":
      return `Reescreva melhorando clareza e gramática, preservando intenção e idioma. ${tone}`;
    case "shorter":
      return `Reescreva de forma mais curta e direta. ${tone}`;
    case "longer":
      return `Expanda com mais contexto e detalhes úteis, sem inventar fatos. ${tone}`;
    case "formal":
      return `Reescreva em tom mais formal. ${tone}`;
    case "casual":
      return `Reescreva em tom mais casual e amigável. ${tone}`;
    case "translate_en":
      return `Traduza para o inglês mantendo o sentido. ${tone}`;
    case "translate_es":
      return `Traduza para o espanhol mantendo o sentido. ${tone}`;
    case "translate_pt":
      return `Traduza para o português mantendo o sentido. ${tone}`;
    case "reply":
      return `Redija uma resposta adequada à mensagem recebida. ${tone}`;
  }
}
