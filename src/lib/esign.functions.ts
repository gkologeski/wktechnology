...
export const updateEsignDocument = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const { error } = await context.supabase.from("esign_documents").update(data.patch).eq("id", data.id);
    if (error) throw new Error(error.message);
    return { ok: true };
  });
...
export const getEsignSession = createServerFn({ method: "GET" })
  .inputValidator((d: { token: string }) => z.object({ token: z.string().min(10).max(100) }).parse(d))
  .handler(async ({ data }) => {
    ...
    // marca como visto
    if (!signer.viewed_at) {
      await supabaseAdmin.from("esign_signers").update({ viewed_at: new Date().toISOString(), status: signer.status === "pending" ? "viewed" : signer.status }).eq("id", signer.id);
      await supabaseAdmin.from("esign_audit").insert({
        document_id: doc.id, signer_id: signer.id, owner_id: signer.owner_id, event: "viewed",
      });
    }
...
export const submitEsignSignature = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const { data: signer, error } = await supabaseAdmin
      .from("esign_signers").select("*").eq("public_token", data.token).maybeSingle();
    ...
    const { error: e2 } = await supabaseAdmin.from("esign_signers").update({
      status: "signed",
    ...
    await supabaseAdmin.from("esign_audit").insert({
      document_id: signer.document_id, signer_id: signer.id, owner_id: signer.owner_id,
      event: "signed", ip_address: ip, user_agent: ua,
    });
...
export const declineEsign = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    if (error || !signer) throw new Error("Link inválido.");
    const { error: e2 } = await supabaseAdmin.from("esign_signers").update({
      status: "declined", decline_reason: data.reason ?? null,
      declined_at: new Date().toISOString(), ip_address: ip, user_agent: ua,
    }).eq("id", signer.id);
    if (e2) throw new Error(e2.message);
    await supabaseAdmin.from("esign_audit").insert({
      document_id: signer.document_id, signer_id: signer.id, owner_id: signer.owner_id,
      event: "declined", ip_address: ip, user_agent: ua,
