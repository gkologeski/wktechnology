...
async function lushaPersonByEmail(email: string) {
  ...
  if (!key) throw new Error("Lusha não conectado: configure a LUSHA_API_KEY na tela de integrações.");
...
export const enrichWithLusha = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    ...
    const { data: job } = await supabase.from("enrichment_jobs").insert({
      ...
      entity: data.entity, status: "running",
      total: rows?.length ?? 0, started_at: new Date().toISOString(),
    ...
    if (credits > 0) {
      await supabase.from("credit_ledger").insert({
        owner_id: userId, provider: "lusha", job_id: job!.id, delta: credits,
