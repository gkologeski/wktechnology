...
const STOPWORDS = new Set([
  "a","o","as","os","de","da","do","das","dos","e","ou","para","por","com","sem","em","no","na","nos","nas",
  "um","uma","uns","umas","que","se","ao","aos","à","às","é","ser","sou","são","foi","ter","ter","tem","esse",
  "isso","isto","esta","este","estes","estas","meu","minha","seu","sua","the","and","or","of","to","in","on",
  "at","for","with","is","are","be","this","that","my","your",
]);
...
export const suggestKbArticles = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: rows, error } = await supabaseAdmin.from("kb_articles")
      .select("id, title, slug, excerpt, body, views")
      .eq("owner_id", ws).eq("published", true).limit(200);
    ...
    const scored = (rows ?? []).map((a) => {
      ...
      return { id: a.id, title: a.title, slug: a.slug, excerpt: a.excerpt, views: a.views, score };
    }).filter((a) => a.score > 0)
