...
const AvailabilitySchema = z.object(
  Object.fromEntries(WeekdayKeys.map((k) => [k, z.array(WindowSchema).max(8).optional()])) as Record<
    typeof WeekdayKeys[number],
    z.ZodOptional<z.ZodArray<typeof WindowSchema>>
  >,
).partial();

const PageInput = z.object({
  id: z.string().uuid().optional(),
  slug: z.string().min(2).max(80).regex(/^[a-z0-9][a-z0-9-]*$/i),
...
export const listBookingPages = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const { data, error } = await supabase.from("booking_pages")
...
export const upsertBookingPage = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: row, error } = await supabase.from("booking_pages").insert(payload).select("id").single();
    if (error) throw new Error(error.message);
    return { id: row!.id };
  });
...
export const listBookings = createServerFn({ method: "GET" })
  .middleware([requireSupabaseAuth])
  .inputValidator((input: unknown) => z.object({ page_id: z.string().uuid().optional() }).default({}).parse(input))
  .handler(async ({ data, context }) => {
    let q = context.supabase.from("bookings").select("*, booking_pages(title,slug)")
...
export const cancelBooking = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const { error } = await context.supabase.from("bookings")
