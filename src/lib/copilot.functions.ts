...
export const askCopilot = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const [contacts, deals, leads, activities] = await Promise.all([
      ...
      supabase
        .from("deals")
        .select("id, name, value, stage, expected_close_date")
        .ilike("name", ilike)
        .limit(5),
      supabase
        .from("leads")
        .select("id, email, phone, source, status")
        .ilike("email", ilike)
        .limit(5),
      supabase
        .from("activities")
    ...
    (contacts.data ?? []).forEach((r) => {
      ...
      sources.push({
        ...
        id: r.id,
        title: String(name),
        snippet: [r.email, r.phone].filter(Boolean).join(" · "),
        url: `/contacts/${r.id}`,
      });
    });
    (deals.data ?? []).forEach((r) =>
      sources.push({
        kind: "deal",
        id: r.id,
        title: r.name,
        snippet: `R$ ${r.value} · ${r.stage}`,
        url: `/deals/${r.id}`,
      }),
    );
    (leads.data ?? []).forEach((r) =>
      sources.push({
        ...
        id: r.id,
        title: r.email ?? "Lead",
        snippet: [r.source, r.status].filter(Boolean).join(" · "),
        url: `/leads/${r.id}`,
      }),
    );
    (activities.data ?? []).forEach((r) =>
      sources.push({
        kind: "activity",
        id: r.id,
        title: r.subject ?? r.type ?? "Atividade",
    ...
    const ctx = sources
      .slice(0, 12)
      .map((s, i) => `[${i + 1}] (${s.kind}) ${s.title} — ${s.snippet}`)
      .join("\n");
