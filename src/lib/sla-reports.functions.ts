...
export const getSlaSummary = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    let q = supabase
      .from("tickets")
      .select(
        "id, created_at, resolved_at, sla_first_response_at, sla_first_response_due_at, sla_resolution_due_at, sla_first_response_breached, sla_resolution_breached, assignee_id, pipeline_id",
      )
      .is("deleted_at", null)
    ...
    for (const t of withFr) {
      const d =
        (new Date(t.sla_first_response_at!).getTime() - new Date(t.created_at).getTime()) / 60000;
      avgFrMin += d;
    }
...
export const getSlaOffenders = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    for (const r of rows ?? []) {
      ...
      a.total++;
      if (isBreached) a.breached++;
      byAgent.set(aKey, a);
      const p = byPipe.get(pKey) ?? { total: 0, breached: 0 };
      p.total++;
      if (isBreached) p.breached++;
      byPipe.set(pKey, p);
    }
    const agents = [...byAgent.entries()]
      .map(([k, v]) => ({ key: k, ...v, pct: v.total ? (v.breached / v.total) * 100 : 0 }))
    ...
    const [agentsData, pipesData] = await Promise.all([
      ...
      pipeIds.length
        ? supabase.from("pipelines").select("id, name").in("id", pipeIds)
    ...
    return {
      agents: agents.map((a) => ({
        ...a,
        label: a.key === "—" ? "Sem responsável" : (aMap.get(a.key) ?? a.key.slice(0, 8)),
      })),
      pipelines: pipelines.map((p) => ({
        ...p,
        label: p.key === "—" ? "Sem fila" : (pMap.get(p.key) ?? p.key.slice(0, 8)),
      })),
    };
  });
