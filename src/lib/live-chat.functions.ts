...
export const convertChatSessionToTicket = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const ws = await resolveActiveWorkspace(context.userId);
    const { data: session, error: sErr } = await supabaseAdmin.from("live_chat_sessions")
      .select("id, visitor_name, visitor_email, visitor_url, status")
      .eq("id", data.session_id).eq("owner_id", ws).maybeSingle();
    if (sErr) throw new Error(sErr.message);
    if (!session) throw new Error("Sessão não encontrada.");

    const { data: msgs, error: mErr } = await supabaseAdmin.from("live_chat_messages")
      .select("direction, body, created_at")
      .eq("session_id", data.session_id).eq("owner_id", ws)
